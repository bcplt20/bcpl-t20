...
const mapStart = content.indexOf('{sec.blocks.map((block, i) => {');
const mapEnd = content.indexOf('</View>\n              </View>\n            </Card>');

const newMap = `{sec.blocks.map((block, i) => {
                          ...
                          {block.items?.map((item: any, idx: number) => (
                            <AmbientShimmerBorder key={idx} style={{ flex: 1 }} colors={[item.color, c.card2, item.color]} innerBg={c.card2} borderRadius={12}>
                              <View style={{ padding: 12, alignItems: 'center' }}>
                                <Text style={{ color: item.color, fontFamily: 'BricolageGrotesque_800ExtraBold', fontSize: 20, marginBottom: 4 }} numberOfLines={1} adjustsFontSizeToFit>{item.v}</Text>
