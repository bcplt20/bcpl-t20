...
- Home reveal system: ALL hide-until-reveal CSS (.rv/.rv-up/.rv-stagger, incl. Home's inline <style> .rv) must be gated on html.rv-js; above-fold elements get .rv-now.rv-in synchronously. LESSON (prod incident 4 Aug '26): IntersectionObserver ALONE is not enough — iOS browsers + late-rendered API content left mid-page sections invisible on bcplt20.com. Reveal must be backed by a scroll/resize + timed-interval sweep that force-adds rv-in to anything near viewport and re-scans for new elements. Never ship hide-by-default CSS without a guaranteed-visibility fallback.
