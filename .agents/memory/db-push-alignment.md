...
3. **New unique constraint on a rowful table** — push interactively offers TRUNCATE (drizzle-kit 0.31). Pre-add via idempotent catch-up SQL (pg_constraint existence check + ADD CONSTRAINT using drizzle's exact `<table>_<col>_unique` name) so push never asks. NEVER trust --force defaults on prod.
