...
**Parallel API+frontend builds drift on contracts.** When backend and frontend subagents build the same feature in parallel, field names/enum values WILL mismatch (budget vs budgetRange, honeypot names, admin PATCH keys). Always diff the two reports' payload shapes and curl the endpoint with the frontend's exact payload before testing UI.
