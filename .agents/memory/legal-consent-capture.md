...
4. `LEGAL_APPROVAL_PENDING` flag in `legalMeta.tsx` gates the draft banners on all legal pages — flip to false + set real effective date only after owner/legal approval, then push (staging-only until then).
