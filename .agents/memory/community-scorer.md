...
**Phase 2 (Aug'26):** raw phone NEVER in any API response (owner sees phoneMasked last-4 only — owner's explicit CricHeroes-style rule); match officials table (scorer/umpire) → canScore()=owner OR scorer official on all 4 scoring mutations; team-add OTP verify (start/confirm, last4+hashed code, 5 attempts/10min, owner team auto-verified) — team_unverified gate must run on ball+undo+innings-end+finish INSIDE the locked tx; community OTP SMS gated (prod or COMMUNITY_SMS_ENABLED=1), devCode only under NODE_ENV=test.
