...
- Asset trap: bcpl-ball-clean.png (brand-book crop) carries a "B/W Logo" caption strip — never use it in UI; the clean mark is bcpl-ball-transparent.png. Similarly bcpl-logo-light/dark sheets vs clean: app uses bcpl-logo-white.png (dark theme) + bcpl-logo-transparent.png (light theme).
- App media tab is fed ONLY by /api/app-media (admin-curated settings key app_media: shorts/videos/photos) — never surface the full website gallery in the app; expo-router Tabs need a real route file even for a FAB placeholder, and hiding a tab is href:null (subagent claimed a fix once but left href:null — verify tab bars visually).
