/**
 * SEO keyword landing pages — shared constants (server side).
 *
 * These four non-brand landing pages (/corporate-cricket, /how-to-join, etc.)
 * exist to rank for searches like "corporate cricket league" and
 * "how to join corporate cricket league india". Each carries a BreadcrumbList
 * (Home → page) and an FAQPage schema built from the SAME question/answer text
 * that the React page renders, so the structured data can never drift from the
...
export interface LandingPage {
  path: string;
  name: string; // breadcrumb label for the page node
  faqs: LandingFaq[];
}

export const LANDING_PAGES: LandingPage[] = [
  {
    path: "/corporate-cricket",
    name: "Corporate Cricket in India",
    faqs: [
      {
  ...
  {
    path: "/corporate-cricket-tournament-delhi",
    name: "Corporate Cricket Tournament in Delhi-NCR",
    faqs: [
      {
  ...
  {
    path: "/how-to-join",
    name: "How to Join a Corporate Cricket League in India",
    faqs: [
      {
  ...
  {
    path: "/office-cricket-team",
    name: "Office Cricket Team Tournament",
    faqs: [
      {
...
export const LANDING_PATHS = new Set(LANDING_PAGES.map((p) => p.path));

/** BreadcrumbList: Home → this page. */
export function breadcrumbLd(page: LandingPage, siteOrigin: string): Record<string, unknown> {
  return {
...
/** FAQPage built from a landing page's own question/answer pairs. */
export function landingFaqLd(page: LandingPage, siteOrigin: string): Record<string, unknown> {
  return {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "@id": `${siteOrigin}${page.path}`,
