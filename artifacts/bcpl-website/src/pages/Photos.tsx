...
.photo-card { position:relative; border-radius:14px; overflow:hidden; cursor:zoom-in; break-inside:avoid; margin-bottom:14px; border:1px solid rgba(255,255,255,0.07); background:#0C271F; }
...
.load-more-btn { background:rgba(212,175,55,0.1); border:1.5px solid rgba(212,175,55,0.45); border-radius:14px; color:#D4AF37; font-family:var(--font-head); font-weight:800; font-size:14px; letter-spacing:.05em; padding:14px 40px; cursor:pointer; transition:all .2s; }
.load-more-btn:hover { background:rgba(212,175,55,0.2); transform:translateY(-2px); }
/* float-reg-btn */
  ...
  return (
    <div style={{minHeight:'100vh',background:'#071B15',fontFamily:'Inter,sans-serif',position:'relative'}}>
      ...
      <section style={{position:'relative',zIndex:1,padding:'20px 0 110px'}}>
        <div className="wrap">
          <div className="glass-card" style={{padding:'clamp(28px,5vw,44px) clamp(20px,5vw,48px)',textAlign:'center',maxWidth:640,margin:'0 auto',border:'1px solid rgba(212,175,55,0.15)',animation:'fadeSlide 0.7s ease both'}}>
            ...
            <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
              <Link href="/videos"
                style={{display:'inline-flex',alignItems:'center',gap:10,padding:'12px 28px',borderRadius:14,background:'linear-gradient(135deg,#D4AF37,#A8842A)',border:'none',color:'#fff',fontFamily:'var(--font-head)',fontWeight:800,fontSize:13,cursor:'pointer',letterSpacing:'0.02em',textDecoration:'none'}}>
