...
.photo-card { position:relative; border-radius:14px; overflow:hidden; cursor:zoom-in; break-inside:avoid; margin-bottom:14px; border:1px solid rgba(12,29,51,0.12); background:#FFFFFF; }
    ...
    <div style={{minHeight:'100vh',background:'#F6F3EC',fontFamily:'Inter,sans-serif',position:'relative'}}>
      ...
      <section style={{position:'relative',zIndex:1,padding:'clamp(80px,12vh,130px) 0 clamp(40px,6vw,64px)',textAlign:'center'}}>
        <div className="wrap">
          ...
          <h1 className="v3-h" style={{fontSize:'clamp(40px,9vw,88px)',marginBottom:20,animation:'floatUp 0.7s ease 0.1s both'}}>
            <span style={{color:'#0C1D33',display:'block'}}>{t("MOMENTS THAT","वो पल जो")}</span>
                ...
                {shown.map((p, i) => (
                  <div key={p.f} className="photo-card" onClick={() => setLightbox({ sec: si, idx: i })}>
                    ...
                    <div className="photo-overlay">
                      <span style={{fontFamily:'var(--font-head)',fontWeight:800,fontSize:11,color:'#0C1D33',letterSpacing:'.08em'}}>
      ...
      {albums.map((album, ai) => (
        <section key={album.id} style={{position:'relative',zIndex:1,padding:'10px 0 60px'}}>
          <div className="wrap">
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10,marginBottom:22}}>
              <div>
                <h2 className="v3-h" style={{fontSize:'clamp(22px,3.5vw,32px)',color:'#0C1D33'}}>{album.name}</h2>
                <div style={{fontFamily:'Inter,sans-serif',fontSize:13,color:'rgba(12,29,51,.60)',marginTop:4}}>
      ...
      <section style={{position:'relative',zIndex:1,padding:'20px 0 110px'}}>
        <div className="wrap">
          <div className="glass-card" style={{padding:'clamp(28px,5vw,44px) clamp(20px,5vw,48px)',textAlign:'center',maxWidth:640,margin:'0 auto',border:'1px solid rgba(255,122,41,0.15)',animation:'fadeSlide 0.7s ease both'}}>
            <h3 className="v3-h" style={{fontSize:'clamp(20px,3vw,28px)',color:'#0C1D33',marginBottom:10}}>
              {t("Auction Videos","Auction Videos")} <span className="shimmer-gold">{t("Are Live","आ गई हैं")}</span>
            </h3>
            <p style={{color:'rgba(12,29,51,.60)',fontSize:14,lineHeight:1.7,maxWidth:460,margin:'0 auto 22px',fontFamily:'Inter,sans-serif'}}>
      ...
      {lightbox !== null && lbSection && lbSection.photos[lightbox.idx] && (
        <div onClick={() => setLightbox(null)}
          ...
          <div style={{position:'absolute',bottom:18,left:'50%',transform:'translateX(-50%)',fontFamily:'var(--font-head)',fontWeight:800,fontSize:12,color:'rgba(12,29,51,.78)',letterSpacing:'.1em'}}>
