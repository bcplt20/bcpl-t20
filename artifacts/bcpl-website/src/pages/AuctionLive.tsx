...
export function AuctionLive() {
  ...
  return (
    <div style={{ background:'#F6F3EC', minHeight: '100vh', fontFamily: 'var(--font-body)', color:'#0C1D33', overflowX: 'hidden' }}>
      <style>{`
      ...
      {/* HERO */}
      <section style={{ padding: '56px 0 0', background: 'linear-gradient(180deg,#F6F3EC 0%,#EDE8DC 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="wrap" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ textAlign: 'center', marginBottom: 44, animation: 'fadeUp 0.5s ease both' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(232,178,61,0.08)', border: '1px solid rgba(232,178,61,0.35)', borderRadius: 12, padding: '8px 20px', marginBottom: 20 }}>
              <span style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 12, color:'#B8892B', letterSpacing: '.14em' }}>
                {t('SEASON 5 · AUGUST 2027', 'SEASON 5 · अगस्त 2027')}
              </span>
            </div>

            <h1 style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 'clamp(26px,5vw,56px)', lineHeight: 1.05, color:'#0C1D33', textTransform: 'uppercase', marginBottom: 12, letterSpacing: '-0.01em' }}>
              {t('THE BCPL', 'BCPL का')}<br />
              <span className="shimmer-gold">{t('PLAYER AUCTION', 'PLAYER AUCTION')}</span>
            </h1>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(14px,2vw,16px)', color:'rgba(12,29,51,.60)', lineHeight: 1.7, maxWidth: 520, margin: '0 auto 28px' }}>
            ...
            <div className="auc-stats">
              {[
                ...
                <div key={i} style={{ background: '#FFFFFF', border: '1px solid rgba(12,29,51,0.12)', borderRadius: 12, padding: '16px 10px' }}>
                  <div style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 'clamp(16px,3vw,24px)', color:'#B8892B', marginBottom: 4 }}>{s.v}</div>
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'rgba(12,29,51,.60)', letterSpacing: '.04em' }}>{t(s.lEn, s.lHi)}</div>
                </div>
              ))}
            </div>
          </div>
        </div>
        <div style={{ height: 48, background: 'linear-gradient(180deg,transparent,#FFFFFF)' }} />
      </section>

      <div className="wrap" style={{ paddingBottom: user ? 60 : 20 }}>
        {/* HOW YOU REACH THE AUCTION */}
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 'clamp(18px,3vw,24px)', color:'#0C1D33', textTransform: 'uppercase', marginBottom: 4 }}>
            {t('How you reach the auction', 'Auction तक कैसे पहुँचते हैं')}
          </div>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'rgba(12,29,51,.60)', marginBottom: 18 }}>
            {t('Four stages — every player follows the same path.', 'चार stages — हर player का रास्ता यही है।')}
          </div>
          <div className="auc-steps">
            {STEPS.map(s => (
              <div key={s.n} style={{ background: '#FFFFFF', border: '1px solid rgba(12,29,51,0.12)', borderRadius: 12, padding: '20px 18px', display: 'flex', gap: 14 }}>
                <div style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 22, color: s.color, flexShrink: 0, lineHeight: 1 }}>{s.n}</div>
                <div>
                  <div style={{ fontFamily: 'var(--font-head)', fontWeight: 800, fontSize: 14, color:'#0C1D33', marginBottom: 6 }}>{t(s.en, s.hi)}</div>
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: 12.5, color: 'rgba(12,29,51,.60)', lineHeight: 1.6 }}>{t(s.dEn, s.dHi)}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* HOW BIDDING WORKS */}
        <div style={{ background: '#FFFFFF', border: '1px solid rgba(12,29,51,0.12)', borderRadius: 12, padding: '24px 20px', marginBottom: 24 }}>
          <div style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 'clamp(16px,2.5vw,20px)', color:'#0C1D33', textTransform: 'uppercase', marginBottom: 12 }}>
            {t('How the bidding works', 'बोली कैसे लगती है')}
          </div>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 13.5, color:'rgba(12,29,51,.60)', lineHeight: 1.8 }}>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid rgba(12,29,51,0.12)', borderRadius: 12, padding: '24px 20px', marginBottom: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
            <div>
              <div style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 'clamp(16px,2.5vw,20px)', color:'#0C1D33', textTransform: 'uppercase' }}>
                {t('Season 4 Auction Gallery', 'Season 4 Auction की झलकियाँ')}
              </div>
              <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'rgba(12,29,51,.60)', marginTop: 2 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 10 }}>
            {[0, 7, 14, 21, 28, 35, 42, 49, 56].map(i => AUCTION_PHOTOS[i] && (
              <Link key={AUCTION_PHOTOS[i].f} href="/photos" style={{ display: 'block', borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(12,29,51,0.12)', lineHeight: 0 }}>
        ...
        {!user && (
          <div style={{ background: 'linear-gradient(135deg,rgba(255,122,41,0.09),rgba(232,178,61,0.05))', border: '1px solid rgba(255,122,41,0.25)', borderRadius: 12, padding: '28px 22px', textAlign: 'center', marginBottom: 60 }}>
            <div style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: 'clamp(17px,3vw,22px)', color:'#0C1D33', textTransform: 'uppercase', marginBottom: 8 }}>
              {t('The road to the auction starts with Phase 1', 'Auction का रास्ता Phase 1 से शुरू होता है')}
            </div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 13.5, color:'rgba(12,29,51,.60)', marginBottom: 18 }}>
