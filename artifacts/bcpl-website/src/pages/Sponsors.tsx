...
const SPONSOR_TIERS = [
  { labelEn:'TITLE SPONSOR', labelHi:'TITLE SPONSOR', color:'#B8892B', descEn:'Exclusive brand visibility across all Season 5 matches, jerseys, and digital platforms.', descHi:'Season 5 के सभी matches, jerseys और digital platforms पर exclusive brand visibility।' },
  ...
  { labelEn:'ASSOCIATE SPONSOR', labelHi:'ASSOCIATE SPONSOR', color:'rgba(12,29,51,.60)', descEn:'Targeted brand exposure across BCPL digital channels and city-level events.', descHi:'BCPL digital channels और city-level events पर targeted brand exposure।' },
];
    ...
    <div style={{background:'#F6F3EC',color:'#0C1D33',minHeight:'100vh',overflowX:'hidden',fontFamily:'Inter,sans-serif'}}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .lbl{font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:rgba(12,29,51,.60);margin-bottom:8px;display:block;font-family:var(--font-head);}
      ...
      <div style={{position:'relative',zIndex:1}}>
        ...
        <section style={{padding:'clamp(80px,12vh,130px) 0 clamp(40px,6vw,64px)',textAlign:'center'}}>
          <div className="wrap">
            ...
            <h1 className="v3-h" style={{fontSize:'clamp(40px,9vw,88px)',marginBottom:20}}>
              <span style={{color:'#0C1D33',display:'block'}}>{t("THE BRANDS","THE BRANDS")}</span>
              <span className="shimmer-gold" style={{display:'block'}}>{t("BEHIND THE DREAM.","BEHIND THE DREAM.")}</span>
            </h1>
            <p style={{color:'rgba(12,29,51,.78)',fontSize:'clamp(14px,2vw,17px)',lineHeight:1.7,maxWidth:640,margin:'0 auto',fontFamily:'Inter,sans-serif'}}>
        ...
        <section style={{padding:'0 0 64px'}}>
          <div className="wrap">
            <div style={{textAlign:'center',marginBottom:40}}>
              <div style={{fontFamily:'var(--font-head)',fontWeight:900,fontSize:16,color:'#0C1D33',marginBottom:8}}>{t("Sponsorship Opportunities — Season 5","Sponsorship Opportunities — Season 5")}</div>
              <p style={{color:'rgba(12,29,51,.60)',fontSize:14,fontFamily:'Inter,sans-serif',maxWidth:480,margin:'0 auto'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr',gap:20}}>
              {SPONSOR_TIERS.map((tier,i)=>(
                <div key={tier.labelEn} style={{background:'#FFFFFF',backdropFilter:'blur(24px)',border:`1.5px dashed ${tier.color}55`,borderRadius:20,padding:'28px 24px',display:'flex',alignItems:'center',gap:20,flexWrap:'wrap'}}>
                  ...
                  <div style={{flex:1,minWidth:200}}>
                    <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8,flexWrap:'wrap'}}>
                      ...
                      <span style={{background:'rgba(12,29,51,0.04)',border:'1px solid rgba(12,29,51,0.12)',color:'rgba(12,29,51,.60)',fontSize:9,fontWeight:700,padding:'2px 8px',borderRadius:100,fontFamily:'var(--font-head)'}}>{t("SLOT AVAILABLE","SLOT AVAILABLE")}</span>
                    </div>
                    <p style={{color:'rgba(12,29,51,.60)',fontSize:13,lineHeight:1.6,fontFamily:'Inter,sans-serif',margin:0}}>{t(tier.descEn,tier.descHi)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section style={{padding:'0 0 80px'}}>
          <div className="wrap">
            <div style={{background:'#FFFFFF',backdropFilter:'blur(32px)',border:'1px solid rgba(255,122,41,0.2)',borderRadius:24,padding:'48px 32px',animation:'borderGlow 3s ease-in-out infinite'}}>
              <div style={{textAlign:'center',marginBottom:40}}>
                <div className="v3-kicker" style={{marginBottom:16}}>{t("PARTNER WITH US","हमारे साथ PARTNER करें")}</div>
                <h2 className="v3-h" style={{fontSize:'clamp(26px,4vw,44px)',color:'#0C1D33',marginBottom:12}}>{t("Become a Sponsor","Sponsor बनें")}</h2>
                <p style={{color:'rgba(12,29,51,.60)',fontSize:15,lineHeight:1.7,maxWidth:480,margin:'0 auto',fontFamily:'Inter,sans-serif'}}>
              ...
              <div className="stats-row" style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:14,marginBottom:40}}>
                {[
                  ...
                  <div key={i} style={{background:'rgba(255,122,41,0.06)',border:'1px solid rgba(255,122,41,0.15)',borderRadius:14,padding:'20px 16px',textAlign:'center'}}>
                    ...
                    <div style={{color:'rgba(12,29,51,.60)',fontSize:12,fontFamily:'Inter,sans-serif',letterSpacing:'0.04em'}}>{t(s.labelEn,s.labelHi)}</div>
                  </div>
                ))}
              </div>
              ...
              {sent?(
                <div style={{textAlign:'center',padding:'40px 20px'}}>
                  ...
                  <p style={{color:'rgba(12,29,51,.60)',fontFamily:'Inter,sans-serif'}}>{t("Our partnerships team will reach out to you within 48 hours.","हमारी partnerships team 48 hours के अंदर आपसे contact करेगी।")}</p>
                </div>
              ):(
                <div>
