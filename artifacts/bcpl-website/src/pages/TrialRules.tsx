...
export function TrialRules() {
  ...
  const css = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .toc-link{color:rgba(12,29,51,.78);text-decoration:none;font-size:13px;font-family:Inter,sans-serif;padding:8px 12px;border-radius:8px;display:flex;align-items:center;gap:8px;transition:all 0.2s;cursor:pointer;background:none;border:none;text-align:left;width:100%;min-height:44px;}
  ...
  const sections:{n:number;icon:IcoComp;title:string;items:React.ReactNode[]}[]=[
    {n:1,icon:IcoTarget,title:'Purpose & Scope',items:[
      ...
      <>This document is separate from the <Link href="/cricket-rulebook" style={{color:'#B8892B',fontWeight:600}}>BCPL Cricket Rulebook</Link>, which governs tournament matches. Trial rules govern trial day only.</>,
    ...
    {n:3,icon:IcoTicket,title:'Check-in, QR Trial Pass & Identity Verification',items:[
      <>Check-in requires your <Link href="/trial-pass" style={{color:'#B8892B',fontWeight:600}}>QR Trial Pass</Link> (available on your dashboard) plus a valid government photo ID consistent with your registration/KYC details.</>,
      ...
      <>If your QR Trial Pass does not scan, do not leave — approach check-in staff, who follow an authorised manual verification process. If a genuine issue remains, email <strong style={{color:'#B8892B'}}>support@bcplt20.com</strong> with your Registration Number.</>,
    ]},
    ...
    {n:13,icon:IcoList,title:'Conduct, Safety, Equipment & Clothing',items:[
      <>Player conduct at trials is governed by the <Link href="/code-of-conduct" style={{color:'#B8892B',fontWeight:600}}>Code of Conduct</Link>. Misconduct, abuse of officials, unfair means or attempts to manipulate the process can lead to disqualification under the published disciplinary process.</>,
    ...
    {n:14,icon:IcoFlag,title:'Weather, Emergencies, Absence & Technical Re-trials',items:[
      ...
      <>Absence / no-show: missing your allotted slot without an approved reschedule means your trial is not completed. Fee treatment follows the <Link href="/refunds" style={{color:'#B8892B',fontWeight:600}}>Refund & Cancellation Policy</Link>.</>,
      ...
      <>Grievance: if you believe a trial-day process error affected your assessment, raise it through the reporting channel below and the <Link href="/contact" style={{color:'#B8892B',fontWeight:600}}>official support channel</Link>. This covers process errors, not disagreement with an evaluator\u2019s cricket judgement.</>,
    ]},
  ];
        ...
        <section style={{padding:'clamp(40px,8vw,72px) 0 40px',textAlign:'center',animation:'fadeSlide 0.6s ease both'}}>
          <div className="wrap">
            ...
            <h1 style={{fontFamily:'Montserrat,sans-serif',fontWeight:900,fontSize:'clamp(32px,7vw,72px)',lineHeight:1.05,marginBottom:8}}>
              <span style={{color:'#0C1D33',display:'block'}}>PHASE 2 — PHYSICAL</span>
              <span className="shimmer-gold" style={{display:'block'}}>TRIAL RULES.</span>
            </h1>
            <p style={{color:'rgba(12,29,51,.60)',fontSize:13,fontWeight:600,letterSpacing:'0.05em',marginTop:16,fontFamily:'Montserrat,sans-serif'}}>Season 5 · Summary of the standardised trial process</p>
            <p style={{color:'rgba(12,29,51,.60)',fontSize:12,marginTop:6,fontFamily:'Inter,sans-serif'}}>यह दस्तावेज़ English में मान्य है · This document is authoritative in English.</p>
            <p style={{color:'rgba(12,29,51,.78)',fontSize:'clamp(14px,2vw,16px)',lineHeight:1.7,maxWidth:640,margin:'16px auto 0'}}>
        ...
        <div className="wrap" style={{maxWidth:900,margin:'0 auto',paddingBottom:40}}>
          ...
          <div className="glass-card" style={{padding:'clamp(20px,4vw,28px) clamp(16px,4vw,32px)',marginBottom:24,animation:'fadeSlide 0.5s ease 0.08s both'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
              <span style={{color:'#B8892B',display:'inline-flex',alignItems:'center'}}><IcoTarget size={20}/></span>
          ...
          <div className="glass-card" style={{padding:'clamp(20px,4vw,28px) clamp(16px,4vw,32px)',marginBottom:24,animation:'fadeSlide 0.5s ease 0.1s both'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
              <span style={{color:'#B8892B',display:'inline-flex',alignItems:'center'}}><IcoPages size={20}/></span>
              <h2 style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:18,color:'#B8892B'}}>Table of Contents</h2>
            </div>
          ...
          {sections.map((sec,idx)=>(
            <div key={sec.n} className="glass-card" style={{padding:'clamp(20px,4vw,32px) clamp(16px,4vw,36px)',marginBottom:20,animation:`fadeSlide 0.5s ease ${0.1+idx*0.06}s both`,border:activeSection===sec.n?'1px solid rgba(255,122,41,0.5)':'1px solid rgba(255,255,255,0.09)',transition:'border-color 0.3s'}}>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:18,flexWrap:'wrap'}}>
                ...
                <h2 style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:'clamp(16px,3vw,20px)',color:'#0C1D33'}}>{sec.title}</h2>
              </div>
              ...
              {sec.n===7 && (
                <div className="rubric-grid" style={{marginTop:18}}>
                  {rubrics.map(r=>(
                    <div key={r.role} style={{background:'rgba(12,29,51,0.04)',border:'1px solid rgba(12,29,51,0.12)',borderRadius:14,padding:'16px 18px'}}>
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                        <span style={{color:'#B8892B',display:'inline-flex',alignItems:'center'}}><r.icon size={20}/></span>
                        <span style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:14,color:'#B8892B',letterSpacing:'.04em'}}>{r.role}</span>
                        <span style={{marginLeft:'auto',fontFamily:'Montserrat,sans-serif',fontWeight:900,fontSize:12,color:'rgba(12,29,51,.60)'}}>/ 100</span>
                      </div>
                      {r.cats.map(k=>(
                        <div key={k} className="rubric-row">
                          <span style={{color:'rgba(12,29,51,.78)'}}>{k}</span>
                          <span style={{color:'rgba(12,29,51,.60)',fontWeight:700,flexShrink:0,fontSize:11}}>criterion</span>
                        </div>
                      ))}
                      <p style={{fontSize:11,color:'rgba(12,29,51,.60)',lineHeight:1.6,marginTop:10}}>{r.note}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              ...
              <div>
                ...
                <p style={{color:'#0C1D33',fontSize:'clamp(13px,2vw,14px)',lineHeight:1.7}}>
                  This page is a plain-language summary of the BCPL physical-trial process. The <strong style={{color:'#B8892B'}}>final published BCPL trial rulebook</strong>, official trial-day briefing and published season rules prevail over this summary wherever they differ. Rubric weights, delivery protocols, allocations and tie-break criteria are versioned and finalised by BCPL Cricket Operations before trials.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card" style={{padding:'clamp(20px,4vw,32px)',textAlign:'center'}}>
            ...
            <p style={{color:'rgba(12,29,51,.78)',fontSize:14,marginBottom:20}}>Phase 1 is open — register, submit your cricket video and take your shot at the BCPL stage.</p>
