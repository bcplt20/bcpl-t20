    ...
    <div style={{background:'#071B15',minHeight:'100vh',fontFamily:'Inter,sans-serif',color:'#F8F4EE',paddingBottom:80,overflowX:'hidden'}}>
      ...
      <div style={{position:'relative',zIndex:1}}>
        ...
        <div className="wrap" style={{maxWidth:900,margin:'0 auto',paddingBottom:40}}>
          ...
          {/* Rule Sections */}
          {rules.map((rule,idx)=>(
            <div key={rule.n} className="glass-card" style={{padding:'clamp(20px,4vw,32px) clamp(16px,4vw,36px)',marginBottom:20,animation:`fadeSlide 0.5s ease ${0.1+idx*0.07}s both`,border:activeSection===rule.n?'1px solid rgba(212,175,55,0.5)':'1px solid rgba(255,255,255,0.09)',transition:'border-color 0.3s'}}>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:18,flexWrap:'wrap'}}>
                <div style={{width:36,height:36,borderRadius:'50%',background:'linear-gradient(135deg,rgba(212,175,55,0.3),rgba(232,178,61,0.2))',border:'1px solid rgba(212,175,55,0.4)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Montserrat,sans-serif',fontWeight:900,fontSize:14,color:'#D4AF37',flexShrink:0}}>{rule.n}</div>
                <span style={{color:'#D4AF37',display:'inline-flex',alignItems:'center'}}><rule.icon size={24}/></span>
          ...
          <div style={{background:'rgba(212,175,55,0.08)',border:'1px solid rgba(212,175,55,0.4)',borderLeft:'3px solid #D4AF37',borderRadius:16,padding:'20px clamp(16px,4vw,24px)',marginBottom:20,animation:'borderGlow 3s ease-in-out infinite'}}>
            <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              ...
              <div>
                <div style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:15,color:'#D4AF37',marginBottom:6}}>Governing Foundation & Hierarchy</div>
                <p style={{color:'rgba(255,255,255,0.85)',fontSize:'clamp(13px,2vw,14px)',lineHeight:1.7}}>
