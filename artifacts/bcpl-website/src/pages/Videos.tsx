...
.play-btn { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:44px; height:44px; border-radius:50%; background:rgba(0,0,0,0.6); display:flex; align-items:center; justify-content:center; transition:all 0.2s; backdrop-filter:blur(4px); border:1.5px solid rgba(12,29,51,0.12); }
.video-card { cursor:pointer; transition:transform 0.2s; }
.video-card:hover { transform:translateY(-4px); }
.lb-btn { background:rgba(12,29,51,0.04); border:1px solid rgba(12,29,51,0.12); border-radius:12px; color:#0C1D33; cursor:pointer; font-size:20px; line-height:1; padding:12px 16px; transition:background .2s; }
.lb-btn:hover { background:rgba(12,29,51,0.04); }
/* float-reg-btn */
      ...
      <section style={{position:'relative',zIndex:1,padding:'clamp(80px,12vh,130px) 0 clamp(40px,6vw,64px)',textAlign:'center'}}>
        <div className="wrap">
          ...
          <h1 className="v3-h" style={{fontSize:'clamp(40px,9vw,88px)',marginBottom:20,animation:'floatUp 0.7s ease 0.1s both'}}>
            <span style={{color:'#0C1D33',display:'block'}}>{t("WATCH EVERY","हर पल")}</span>
      ...
      <section style={{position:'relative',zIndex:1,padding:'0 0 56px'}}>
        <div className="wrap">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10,marginBottom:22}}>
            <div>
              <h2 className="v3-h" style={{fontSize:'clamp(22px,3.5vw,32px)',color:'#0C1D33'}}>
                {t("Season 4 Auction","Season 4 Auction")}
              </h2>
              <div style={{fontFamily:'Inter,sans-serif',fontSize:13,color:'rgba(12,29,51,.60)',marginTop:4}}>
          ...
          <div className="feat-card" onClick={() => setModal({ kind: 'yt', id: AUCTION_STREAM.ytId })}
            style={{aspectRatio:'16/9',maxHeight:520,width:'100%',background:'#FFFFFF',marginBottom:14}}>
            ...
            <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'0 18px 18px'}}>
              ...
              <div style={{fontFamily:'Inter,sans-serif',fontSize:13,color:'rgba(255,255,255,0.82)',marginTop:4}}>
          ...
          <div style={{textAlign:'right'}}>
            <a href={`https://youtu.be/${AUCTION_STREAM.ytId}`} target="_blank" rel="noopener noreferrer"
              style={{color:'rgba(12,29,51,.60)',fontSize:13,fontFamily:'Inter,sans-serif',textDecoration:'none'}}>
      ...
      <section style={{position:'relative',zIndex:1,padding:'0 0 90px'}}>
        <div className="wrap">
          <div style={{marginBottom:18}}>
            <h3 className="v3-h" style={{fontSize:'clamp(18px,2.6vw,24px)',color:'#0C1D33'}}>
          ...
          <div className="video-grid">
            {AUCTION_CLIPS.map((c,i)=>(
              <div key={c.file} className="video-card" style={{animation:`fadeSlide 0.5s ease ${(i%3)*0.1}s both`}} onClick={()=>openClip(c)}>
                <div className="video-thumb" style={{aspectRatio:'16/9',background:'#FFFFFF',marginBottom:12}}>
                ...
                <div>
                  <h3 style={{fontFamily:'var(--font-head)',fontWeight:700,fontSize:15,color:'#0C1D33',marginBottom:6,lineHeight:1.4}}>{t(c.title, c.titleHi)}</h3>
                  <div style={{display:'flex',alignItems:'center',gap:6}}>
                    ...
                    <span style={{color:'rgba(12,29,51,.60)',fontSize:12,fontFamily:'Inter,sans-serif'}}>BCPL TV</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section style={{position:'relative',zIndex:1,padding:'0 0 120px'}}>
        <div className="wrap">
          <div className="glass-card" style={{padding:'clamp(20px,5vw,48px) clamp(16px,4vw,48px)',textAlign:'center',maxWidth:640,margin:'0 auto',border:'1px solid rgba(255,122,41,0.15)'}}>
            ...
            <h3 className="v3-h" style={{fontSize:28,color:'#0C1D33',marginBottom:6}}>{t("Subscribe to BCPL TV","BCPL TV Subscribe करें")}</h3>
            <div style={{color:'rgba(12,29,51,.60)',fontSize:14,marginBottom:24,fontFamily:'Inter,sans-serif'}}>{t("23K subscribers · Match highlights & league videos","23K subscribers · Match highlights और league videos")}</div>
            <div style={{display:'flex',gap:14,justifyContent:'center',flexWrap:'wrap'}}>
              ...
              <a href="https://www.youtube.com/@bcplt20league?sub_confirmation=1" target="_blank" rel="noopener noreferrer" style={{padding:'14px 28px',borderRadius:14,background:'rgba(12,29,51,0.04)',border:'1.5px solid rgba(12,29,51,0.12)',color:'rgba(12,29,51,.78)',fontFamily:'var(--font-head)',fontWeight:700,fontSize:15,cursor:'pointer',textDecoration:'none',display:'inline-flex',alignItems:'center',gap:8}}>
