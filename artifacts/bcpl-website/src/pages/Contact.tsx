    ...
    <div style={{minHeight:'100vh',background:'#F6F3EC',fontFamily:'Inter,sans-serif',position:'relative'}}>
      ...
      <section style={{position:'relative',zIndex:1,padding:'clamp(80px,12vh,130px) 0 clamp(48px,7vw,80px)',textAlign:'center'}}>
        <div className="wrap">
          ...
          <h1 className="v3-h" style={{fontSize:'clamp(40px,9vw,88px)',marginBottom:20,animation:'floatUp 0.7s ease 0.1s both'}}>
            <span style={{color:'#0C1D33',display:'block'}}>{t("WE'RE HERE","हम यहां हैं")}</span>
        ...
        <div className="wrap">
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:24}}>
            ...
            <div className="glass-card contact-method-card" style={{padding:'36px 28px',border:'1px solid rgba(37,211,102,0.25)',boxShadow:'0 0 40px rgba(37,211,102,0.06)',animation:'fadeSlide 0.6s ease 0.1s both'}}>
              ...
              <div style={{fontFamily:'var(--font-head)',fontWeight:900,fontSize:20,color:'#0C1D33',marginBottom:8}}>WhatsApp</div>
              <div style={{color:'rgba(12,29,51,.60)',fontSize:14,lineHeight:1.6,marginBottom:8}}>{t("Fastest response.","सबसे तेज़ जवाब।")}</div>
            ...
            <div className="glass-card contact-method-card" style={{padding:'36px 28px',border:'1px solid rgba(6,182,212,0.25)',boxShadow:'0 0 40px rgba(6,182,212,0.06)',animation:'fadeSlide 0.6s ease 0.2s both'}}>
              ...
              <div style={{fontFamily:'var(--font-head)',fontWeight:900,fontSize:20,color:'#0C1D33',marginBottom:8}}>Email</div>
              <div style={{color:'#06B6D4',fontSize:15,fontWeight:700,marginBottom:4}}>support@bcplt20.com</div>
              <div style={{color:'rgba(12,29,51,.60)',fontSize:13,lineHeight:1.6,marginBottom:24}}>{t("Response within 24 hours","24 घंटे में जवाब")}</div>
            ...
            <div className="glass-card contact-method-card" style={{padding:'36px 28px',border:'1px solid rgba(139,92,246,0.25)',boxShadow:'0 0 40px rgba(139,92,246,0.06)',animation:'fadeSlide 0.6s ease 0.3s both'}}>
              ...
              <div style={{fontFamily:'var(--font-head)',fontWeight:900,fontSize:20,color:'#0C1D33',marginBottom:8}}>{t("Call Us","Call करें")}</div>
              <div style={{color:'#8B5CF6',fontSize:15,fontWeight:700,marginBottom:4}}>+91 91513 46555</div>
              <div style={{color:'rgba(12,29,51,.60)',fontSize:13,lineHeight:1.6,marginBottom:24}}>{t("Mon–Sat 10AM–7PM IST","सोम–शनि 10AM–7PM IST")}</div>
      ...
      `}</style>
            ...
            <div className="glass-card" style={{padding:'clamp(20px,5vw,40px)',animation:'fadeSlide 0.7s ease 0.2s both'}}>
              ...
              <h2 className="v3-h" style={{fontSize:30,color:'#0C1D33',marginBottom:8}}>{t("Drop Us a Line","हमें लिखें")}</h2>
              <p style={{color:'rgba(12,29,51,.60)',fontSize:14,marginBottom:32,lineHeight:1.6}}>{t("Fill out the form and our team will get back to you within 24 hours.","Form भरें और हमारी team 24 घंटे में आपसे संपर्क करेगी।")}</p>
              ...
              {sent ? (
                <div style={{textAlign:'center',padding:'40px 20px'}}>
                  ...
                  <div style={{color:'rgba(12,29,51,.78)',fontSize:14}}>{t("Your email app should open with the details pre-filled. Just review and send it to support@bcplt20.com.","आपका email app details के साथ खुलना चाहिए। बस review करके support@bcplt20.com पर भेज दें।")}</div>
                </div>
              ) : (
                <form onSubmit={handleSubmit}>
                  ...
                  <div style={{marginBottom:16}}>
                    ...
                    <select className="inp" value={form.subject} onChange={e=>setForm(f=>({...f,subject:e.target.value}))} style={{color:form.subject?'#0C1D33':'rgba(12,29,51,.60)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:20}}>
              <div className="glass-card" style={{padding:'32px',animation:'fadeSlide 0.7s ease 0.35s both'}}>
                ...
                <div style={{fontFamily:'var(--font-head)',fontWeight:800,fontSize:18,color:'#0C1D33',marginBottom:8}}>{t("Common Questions?","आम सवाल?")}</div>
                <p style={{color:'rgba(12,29,51,.60)',fontSize:14,marginBottom:20,lineHeight:1.6}}>{t("Quick answers to the most frequent queries:","सबसे frequent queries के quick answers:")}</p>
              ...
              <div className="glass-card" style={{padding:'28px',animation:'fadeSlide 0.7s ease 0.45s both'}}>
                <div style={{fontFamily:'var(--font-head)',fontWeight:800,fontSize:15,color:'rgba(12,29,51,.78)',marginBottom:16,display:'flex',alignItems:'center',gap:8}}><IcoPin size={16}/> Find Us</div>
                <div style={{color:'rgba(12,29,51,.60)',fontSize:13,lineHeight:1.8}}>
                  <div>Kriparthi Playing 11 Pvt. Ltd.</div>
                  <div style={{color:'rgba(12,29,51,.60)'}}>{t("Operating BCPL — Bhartiya Corporate Premier League","BCPL — Bhartiya Corporate Premier League का संचालन")}</div>
              ...
              <div className="glass-card" style={{padding:'28px',animation:'fadeSlide 0.7s ease 0.55s both'}}>
                <div style={{fontFamily:'var(--font-head)',fontWeight:800,fontSize:15,color:'rgba(12,29,51,.78)',marginBottom:16,display:'flex',alignItems:'center',gap:8}}><IcoClock size={16}/> Support Hours</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                  {[['Mon–Fri','10AM–7PM'],['Saturday','10AM–5PM'],['Sunday','Closed']].map(([d,h])=>(
                    <div key={d}>
                      <div style={{color:'rgba(12,29,51,.60)',fontSize:11,fontFamily:'var(--font-head)',fontWeight:700}}>{d}</div>
                      <div style={{color:'rgba(12,29,51,.78)',fontSize:13,fontWeight:600}}>{h}</div>
                    </div>
                  ))}
                </div>
                <p style={{color:'rgba(12,29,51,.60)',fontSize:11.5,marginTop:12,lineHeight:1.6}}>{t("Times are IST. Messages sent outside these hours are answered on the next working day.","समय IST में हैं। इन घंटों के बाहर भेजे गए संदेश अगले कार्यदिवस पर उत्तरित किए जाते हैं।")}</p>
              </div>
            </div>
          </div>
        </div>
