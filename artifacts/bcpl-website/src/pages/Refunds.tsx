...
export function Refunds() {
  ...
  const css = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .lbl{font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:rgba(12,29,51,.60);margin-bottom:8px;display:block;}
    ...
    .toc-link{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;text-decoration:none;color:rgba(12,29,51,.78);font-size:13px;border:1px solid transparent;transition:all .2s;min-height:40px;}
  ...
  const toneColors:Record<string,{bg:string;bd:string;fg:string}> = {
    ...
    gold:{bg:'rgba(232,178,61,0.08)',bd:'rgba(232,178,61,0.4)',fg:'#B8892B'},
    blue:{bg:'rgba(59,130,246,0.08)',bd:'rgba(59,130,246,0.3)',fg:'#2563EB'},
  };

  const renderBlock = (b: Block, i: number) => {
    if (b.kind === 'p') return (
      <p key={i} style={{color:'rgba(12,29,51,.78)',fontSize:'clamp(13px,2vw,15px)',lineHeight:1.75,marginBottom:12}}>{b.text}</p>
    );
    if (b.kind === 'ul') return (
      <ul key={i} style={{listStyle:'none',display:'flex',flexDirection:'column',gap:8,marginBottom:12}}>
        {b.items.map((it,j)=>(<li key={j} style={{display:'flex',alignItems:'flex-start',color:'rgba(12,29,51,.78)',fontSize:'clamp(13px,2vw,14.5px)',lineHeight:1.7}}><OrangeDot/><span>{it}</span></li>))}
      </ul>
    );
    if (b.kind === 'ol') return (
      <ol key={i} style={{listStyle:'none',display:'flex',flexDirection:'column',gap:8,marginBottom:12,paddingLeft:0}}>
        {b.items.map((it,j)=>(<li key={j} style={{color:'rgba(12,29,51,.78)',fontSize:'clamp(13px,2vw,14.5px)',lineHeight:1.7,paddingLeft:4}}>{it}</li>))}
      </ol>
    );
    // note
    ...
    return (
      <div key={i} style={{background:c.bg,border:`1px solid ${c.bd}`,borderLeft:`4px solid ${c.fg}`,borderRadius:12,padding:'14px 16px',marginBottom:12}}>
        ...
        <p style={{color:'#0C1D33',fontSize:'clamp(13px,2vw,14.5px)',lineHeight:1.7,fontWeight:600}}>{b.text}</p>
      </div>
    );
  };
        ...
        <section style={{padding:'clamp(40px,8vw,72px) 0 40px',textAlign:'center',animation:'fadeSlide 0.6s ease both'}}>
          <div className="wrap">
            ...
            <h1 style={{fontFamily:'Montserrat,sans-serif',fontWeight:900,fontSize:'clamp(32px,7vw,72px)',lineHeight:1.05,marginBottom:8}}>
              <span style={{color:'#0C1D33',display:'block'}}>REFUND &</span>
        ...
        <div className="wrap" style={{maxWidth:900,margin:'0 auto',paddingBottom:40}}>
          ...
          <div className="glass-card" style={{padding:'clamp(20px,4vw,32px) clamp(16px,4vw,36px)',marginBottom:24,animation:'fadeSlide 0.5s ease 0.1s both'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
              <span style={{color:'#B8892B',display:'inline-flex',alignItems:'center'}}><IcoList size={20}/></span>
              <h2 style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:18,color:'#B8892B'}}>Key Points</h2>
            </div>
            <ul style={{listStyle:'none',display:'flex',flexDirection:'column',gap:12}}>
              {[
                ...
                <li key={i} style={{display:'flex',alignItems:'flex-start',gap:10,color:'#0C1D33',fontSize:'clamp(13px,2vw,15px)',lineHeight:1.7}}>
          ...
          <div style={{background:'rgba(232,178,61,0.10)',border:'2px solid rgba(232,178,61,0.45)',borderLeft:'4px solid #E8B23D',borderRadius:16,padding:'18px clamp(16px,4vw,24px)',marginBottom:20,animation:'fadeSlide 0.5s ease 0.15s both'}}>
            <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              <span style={{flexShrink:0,lineHeight:1,color:'#B8892B',display:'inline-flex'}}><IcoWarn size={24}/></span>
              <div>
                <div style={{fontFamily:'Montserrat,sans-serif',fontWeight:900,fontSize:15,color:'#B8892B',marginBottom:6}}>Payment Disclaimer</div>
                <p style={{color:'#0C1D33',fontSize:'clamp(13px,2vw,15px)',lineHeight:1.75,fontWeight:600}}>
          ...
          {/* Fee context note */}
          <div style={{background:'rgba(12,29,51,0.04)',border:'1px solid rgba(12,29,51,0.12)',borderRadius:14,padding:'14px clamp(16px,4vw,22px)',marginBottom:24}}>
            <p style={{color:'rgba(12,29,51,.78)',fontSize:13,lineHeight:1.75}}>
          ...
          <div className="glass-card" style={{padding:'clamp(18px,4vw,26px)',marginBottom:24,animation:'fadeSlide 0.5s ease 0.2s both'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
              ...
              <h2 style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:16,color:'#0C1D33'}}>Contents</h2>
            </div>
            ...
            return (
              <div key={s.n} id={`refund-${s.n}`} className="glass-card" style={{padding:'clamp(16px,4vw,24px) clamp(16px,4vw,30px)',marginBottom:12,scrollMarginTop:90,animation:`fadeSlide 0.4s ease ${Math.min(0.05+idx*0.02,0.4)}s both`}}>
                <button className="acc-head" onClick={()=>setOpen(isOpen?null:s.n)} aria-expanded={isOpen}>
                  ...
                  <h2 style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:'clamp(15px,2.6vw,18px)',color:'#0C1D33',flex:1}}>{s.title}</h2>
                  <span style={{color:'rgba(12,29,51,.60)',fontSize:18,transition:'transform .2s',transform:isOpen?'rotate(180deg)':'none',flexShrink:0}}>▾</span>
                </button>
                {isOpen && (
                  <div style={{marginTop:16,paddingTop:16,borderTop:'1px solid rgba(12,29,51,0.12)'}}>
          ...
          <div className="glass-card" style={{padding:'clamp(20px,4vw,32px) clamp(16px,4vw,36px)',margin:'24px 0',border:'1px solid rgba(255,122,41,0.2)'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20}}>
              ...
              <h2 style={{fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:'clamp(16px,3vw,20px)',color:'#0C1D33'}}>Refund Request Form</h2>
            </div>
            {submitted ? (
              <div style={{textAlign:'center',padding:'24px 0'}}>
                ...
                <p style={{color:'rgba(12,29,51,.78)',fontSize:14}}>We've received your refund request. We will review it against this policy and respond to your registered email.</p>
              </div>
            ) : (
              <>
                ...
                <p style={{textAlign:'center',marginTop:14,color:'rgba(12,29,51,.60)',fontSize:13}}>
                  Or email directly: <strong style={{color:'#B8892B'}}>support@bcplt20.com</strong>
                </p>
              </>
            )}
          </div>
            ...
            <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              ...
              <div>
                ...
                <p style={{color:'#0C1D33',fontSize:'clamp(13px,2vw,14px)',lineHeight:1.7}}>
                  Contact our support team at <strong style={{color:'#B8892B'}}>support@bcplt20.com</strong>. We're here 9 AM – 7 PM, Monday–Saturday.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card" style={{padding:'clamp(20px,4vw,32px)',textAlign:'center'}}>
            ...
            <p style={{color:'rgba(12,29,51,.78)',fontSize:14,marginBottom:20}}>Before you pay, please note that the Phase 1 fee is non-refundable once successfully paid, including if you do not upload your video.</p>
