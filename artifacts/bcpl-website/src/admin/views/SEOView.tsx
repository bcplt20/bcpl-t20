...
export default function SEOView() {
  ...
  if (loading) {
    return <div style={{ padding: 40, color: "#A6B3D0", fontSize: 13 }}>Loading SEO settings…</div>;
  }
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {([["meta", "Meta Tags"], ["sitemap", "Sitemap & Robots"], ["google", "Google Verification"]] as [Tab, string][]).map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)} style={{
            padding: "9px 18px", borderRadius: 9, border: "1px solid " + (tab === t ? "#FF6B00" : "#33436B"),
            background: tab === t ? "#FF6B0015" : "transparent", color: tab === t ? "#FF6B00" : "#C3CEE3",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "230px 1fr", gap: 14, alignItems: "start" }}>
          <div style={{ ...card, padding: 10 }}>
            {data.pages.map((p) => (
              <button key={p.path} onClick={() => pick(p)} style={{
                ...
                <span style={{ fontSize: 12.5, color: selPath === p.path ? "#FF6B00" : "#C3CEE3", fontWeight: selPath === p.path ? 700 : 500 }}>
                  {p.label}
                </span>
                {!p.isDefault && <span title="Customized" style={{ width: 7, height: 7, borderRadius: "50%", background: "#3B82F6", flexShrink: 0 }} />}
              </button>
            ))}
            <div style={{ marginTop: 10, padding: "10px 12px", background: "#243050", borderRadius: 9, border: "1px solid #33436B", fontSize: 10.5, color: "#94A3C4", lineHeight: 1.6 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={card}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "#94A3C4", marginTop: 3, fontFamily: "monospace" }}>{origin}{sel?.path === "/" ? "" : sel?.path}</div>
                </div>
                {sel && !sel.isDefault && (
              ...
              <label style={{ fontSize: 11, fontWeight: 700, color: "#A6B3D0", display: "block", marginBottom: 6 }}>
                Page title <span style={{ color: titleLen > 60 ? "#EF4444" : "#94A3C4", fontWeight: 500 }}>({titleLen}/60)</span>
              </label>
              <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} style={inputStyle} maxLength={120} />

              <label style={{ fontSize: 11, fontWeight: 700, color: "#A6B3D0", display: "block", margin: "14px 0 6px" }}>
                Description <span style={{ color: descLen > 160 ? "#EF4444" : "#94A3C4", fontWeight: 500 }}>({descLen}/160)</span>
              </label>
              ...
              <label style={{ fontSize: 11, fontWeight: 700, color: "#A6B3D0", display: "block", margin: "14px 0 6px" }}>
                Share image URL <span style={{ color: "#94A3C4", fontWeight: 500 }}>(optional — WhatsApp/Facebook preview; leave empty for the BCPL logo)</span>
              </label>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 16, alignItems: "center" }}>
                ...
                <span style={{ fontSize: 11, color: "#94A3C4" }}>Served on the live site within ~1 minute (needs the site deployed once with this feature).</span>
              </div>
            </div>
            ...
            <div style={card}>
              ...
              {checks.map((c) => (
                <div key={c.label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 0", fontSize: 12, color: c.ok ? "#C3CEE3" : "#FCD34D" }}>
      ...
      {tab === "sitemap" && data && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={card}>
            ...
            {[
              ...
              <div key={f.key} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", flexWrap: "wrap" }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3", width: 70 }}>{f.label}</span>
                <code style={{ fontSize: 12, color: "#FF8C40", background: "#243050", padding: "5px 10px", borderRadius: 7, border: "1px solid #33436B" }}>{f.url}</code>
                ...
                <span style={{ fontSize: 11, color: "#94A3C4", flexBasis: "100%" }}>{f.note}</span>
              </div>
            ))}
          </div>
          ...
          <div style={card}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
              {[...data.pages.map((p) => p.path), ...data.teamSlugs.map((s) => `/team/${s}`)].map((u) => (
                <div key={u} style={{ fontSize: 11.5, fontFamily: "monospace", color: "#C3CEE3", padding: "6px 10px", background: "#243050", borderRadius: 7, border: "1px solid #3A4A72" }}>
                  {u}
                </div>
              ))}
            </div>
          ...
          <div style={card}>
            ...
            <ol style={{ margin: 0, paddingLeft: 18, fontSize: 12.5, color: "#C3CEE3", lineHeight: 2 }}>
      ...
      {tab === "google" && data && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 900 }}>
          ...
          <div style={{ ...card, borderLeft: `3px solid ${data.gscCode ? "#10B981" : "#F59E0B"}` }}>
            ...
            {data.gscCode ? (
              <>
                <div style={{ fontSize: 12, color: "#C3CEE3", lineHeight: 1.7, marginBottom: 10 }}>
                  This tag is injected into every page of the live site. Open the site → right-click → View Page Source to see it.
                </div>
                <code style={{ display: "block", fontSize: 11.5, color: "#6EE7B7", background: "#243050", padding: "10px 12px", borderRadius: 8, border: "1px solid #33436B", wordBreak: "break-all" }}>
              ...
              <div style={{ fontSize: 12, color: "#C3CEE3", lineHeight: 1.7 }}>
...
function StatCard({ label, value, delta, deltaGood, hint }: {
  ...
  const dc = deltaGood === undefined ? "#C3CEE3" : deltaGood ? "#10B981" : "#F87171";
  return (
    <div style={{ background: "#243050", border: "1px solid #33436B", borderRadius: 12, padding: "14px 16px", flex: "1 1 150px", minWidth: 140 }}>
      <div style={{ fontSize: 10.5, fontWeight: 700, color: "#A6B3D0", letterSpacing: .5, textTransform: "uppercase" }}>{label}</div>
      <div style={{ fontSize: 24, fontWeight: 800, color: "#F1F5F9", marginTop: 6, lineHeight: 1 }}>{value}</div>
      {delta !== undefined && (
        <div style={{ fontSize: 11.5, color: dc, marginTop: 6 }}>{delta} <span style={{ color: "#94A3C4" }}>{hint ?? "vs prev 28d"}</span></div>
      )}
    </div>
  );
}
...
function MiniTable({ title, colLabel, rows }: {
  ...
  return (
    <div style={{ background: "#243050", border: "1px solid #33436B", borderRadius: 12, padding: 14, flex: "1 1 360px", minWidth: 300 }}>
      <div style={{ fontSize: 12.5, fontWeight: 700, color: "#F1F5F9", marginBottom: 10 }}>{title}</div>
      {rows.length === 0 ? (
        <div style={{ fontSize: 12, color: "#A6B3D0", padding: "8px 0" }}>No data yet for this period.</div>
      ) : (
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
          <thead>
            <tr style={{ textAlign: "left", color: "#94A3C4", fontSize: 10, letterSpacing: .3 }}>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} style={{ borderTop: "1px solid #33436B", color: "#CBD5E1" }}>
...
function GscSetupPanel({ message }: { message: string }) {
  return (
    <div style={{ ...card, borderLeft: "3px solid #F59E0B" }}>
      ...
      <div style={{ fontSize: 11.5, color: "#A6B3D0", marginBottom: 10 }}>{message}</div>
      <div style={{ fontSize: 12.5, color: "#CBD5E1", fontWeight: 600, marginBottom: 6 }}>Setup (ek baar) — 3 steps:</div>
      <ol style={{ margin: 0, paddingLeft: 18, fontSize: 12.5, color: "#C3CEE3", lineHeight: 1.9 }}>
      ...
      <div style={{ fontSize: 11, color: "#94A3C4", marginTop: 10 }}>Ye ho jaane ke baad yahan clicks, impressions aur queries apne-aap dikhne lagenge.</div>
    </div>
  );
}
...
function GscPanel({ gsc, loading, error, onRefresh }: {
  ...
  if (loading && !gsc) {
    return <div style={{ ...card }}><div style={{ fontSize: 12.5, color: "#A6B3D0" }}>Loading Search Console data…</div></div>;
  }
  if (error) {
    ...
    <div style={{ ...card }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#A6B3D0", marginTop: 3 }}>
