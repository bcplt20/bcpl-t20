  ...
  return (
    <div style={{ ...card, borderTop: `3px solid ${statusColor}` }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, flexWrap:"wrap", marginBottom:4 }}>
        <div style={{ fontSize:14, fontWeight:800, color:"#1E293B" }}>AI Evaluation Controls</div>
        {cfg && (
      ...
      {cfgErr && (
        <div style={{ padding:"8px 12px", borderRadius:8, background:"#EF444418", border:"1px solid #EF444440", color:"#DC2626", fontSize:11, marginBottom:8 }}>
          {cfgErr}
        </div>
      )}
      ...
      {cfg && (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {AI_SWITCHES.map((s) => {
            ...
            const onColor = s.key === "testMode" ? "#B45309" : "#10B981";
            return (
              <div key={s.key} style={{ display:"flex", alignItems:"center", gap:14, padding:"10px 14px", background:"#F5F6F8", borderRadius:10, border:"1px solid #E2E8F0" }}>
                ...
                <button onClick={() => flip(s.key)} disabled={savingKey !== null} aria-label={`${s.label}: ${on ? "on" : "off"}`}
                  style={{ width:52, height:28, borderRadius:999, border:`1px solid ${on ? `${onColor}60` : "#E2E8F0"}`,
                    background: on ? `${onColor}30` : "#E2E8F0", position:"relative",
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800, color:"#1E293B" }}>API Health</div>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={()=>load(false)} disabled={loading||probing}
            style={{ padding:"9px 16px", borderRadius:10, border:"1px solid #E2E8F0", background:"transparent", color:"#64748B", fontSize:12, fontWeight:700, cursor:"pointer" }}>
            ↻ Refresh
          </button>
      ...
      {err && (
        <div style={{ padding:"10px 14px", borderRadius:10, background:"#EF444418", border:"1px solid #EF444440", color:"#DC2626", fontSize:12 }}>
          {err}
        </div>
      )}
      ...
      {data && (
        <>
              ...
              return (
                <div key={it.id} style={{ ...card, borderTop:`3px solid ${border}`, display:"flex", flexDirection:"column", gap:8 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                    <div style={{ fontSize:13, fontWeight:800, color:"#1E293B" }}>{it.name}</div>
                    <span style={{ fontSize:10, fontWeight:800, padding:"3px 9px", borderRadius:6,
                      background: it.configured ? "#10B98120" : "#EF444420",
                      color: it.configured ? "#047857" : "#EF4444" }}>
                      {it.configured ? "● Configured" : "○ Not configured"}
                    </span>
                  </div>
                  <div style={{ fontSize:11, color:"#64748B" }}>Last activity: <span style={{ color:"#64748B" }}>{ago(it.lastActivityAt)}</span></div>
                  {it.probe && (
                    <div style={{ fontSize:11, padding:"6px 10px", borderRadius:8,
                      ...
                      color: it.probe.ok ? "#047857" : "#EF4444" }}>
                      Live probe: {it.probe.ok ? "OK" : "FAILED"} — {it.probe.note}
                    </div>
                  )}
                  <div style={{ fontSize:10, color:"#64748B" }}>{it.note}</div>
                </div>
              );
            })}
          </div>
          ...
          {/* Background jobs */}
          <div style={card}>
            <div style={{ fontSize:14, fontWeight:800, color:"#1E293B", marginBottom:12 }}>Background Jobs</div>
            {data.jobs.length === 0 && (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {data.jobs.map(j => {
                ...
                return (
                  <div key={j.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:"#F5F6F8", borderRadius:10, border:`1px solid ${bad ? "#EF444440" : "#E2E8F0"}`, flexWrap:"wrap" }}>
                    <span style={{ width:8, height:8, borderRadius:"50%", background: bad ? "#EF4444" : "#10B981", flexShrink:0 }}/>
                    <span style={{ fontSize:12, fontWeight:700, color:"#1E293B", minWidth:150 }}>{j.id}</span>
                    <span style={{ fontSize:11, color:"#64748B" }}>{fmtInterval(j.intervalMs)}</span>
                    <span style={{ fontSize:11, color:"#64748B" }}>last run: <span style={{ color:"#64748B" }}>{ago(j.lastRunAt)}</span></span>
                    <span style={{ fontSize:11, color:"#64748B" }}>runs: {j.runs} · fails: {j.fails}</span>
                    {jobStale(j) && <span style={{ fontSize:10, fontWeight:800, color:"#B45309" }}>STALE — overdue</span>}
                    {j.lastError && <span style={{ fontSize:10, color:"#DC2626", flexBasis:"100%" }}>Last error: {j.lastError}</span>}
                  </div>
                );
              })}
            </div>
          ...
          {/* AI evaluation queue */}
          <div style={card}>
            <div style={{ fontSize:14, fontWeight:800, color:"#1E293B", marginBottom:12 }}>AI Evaluation Queue</div>
            {queues.length === 0 ? (
              <div style={{ fontSize:12, color:"#475569" }}>No evaluations in the pipeline.</div>
            ) : (
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                {queues.map(q => (
                  <div key={q.label} style={{ padding:"10px 18px", background:"#F5F6F8", borderRadius:10, border:"1px solid #E2E8F0", textAlign:"center" }}>
                    <div style={{ fontSize:20, fontWeight:800, color:"#1E293B", lineHeight:1 }}>{q.count}</div>
