...
const card: React.CSSProperties = { background:"linear-gradient(135deg,#2C3A5E 0%,#1F2B49 100%)", border:"1px solid #33436B", borderRadius:16, padding:"20px 22px" };
...
const ROLE_META: Record<string, { label: string; color: string; hint: string }> = {
  ...
  SUPPORT_TEAM:        { label:"Support Team",          color:"#A6B3D0", hint:"Support tickets and player lookups" },
...
export default function RolesView() {
  ...
  const meta = ROLE_META[selected] ?? { label: selected, color: "#A6B3D0", hint: "" };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      {loading && !err && (
        <div style={{ ...card, textAlign:"center", color:"#94A3C4", fontSize:12, padding:36 }}>Loading roles…</div>
      )}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"280px 1fr", gap:16, alignItems:"start" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ ...card, borderTop:`3px solid ${meta.color}` }}>
              ...
              <div style={{ fontSize:12, color:"#C3CEE3", lineHeight:1.6 }}>{meta.hint}</div>
              <div style={{ marginTop:14 }}>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  ...
                  {views.length === 0 && <span style={{ fontSize:11, color:"#94A3C4" }}>No sections mapped</span>}
                </div>
              </div>
            </div>
            ...
            <div style={card}>
              ...
              {holders.length === 0 ? (
                <div style={{ fontSize:12, color:"#94A3C4" }}>No admin accounts hold this role yet — add one in Admin Management.</div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {holders.map(h => (
                    <div key={h.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:"#243050", borderRadius:10, border:"1px solid #33436B" }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ fontSize:10, color:"#A6B3D0" }}>{h.email}</div>
                      </div>
