...
export default function LeaderboardView() {
  ...
  const tab_btn = (a:boolean): React.CSSProperties => ({ padding:"9px 18px", borderRadius:10, border:`1px solid ${a?"#FF6B00":"#E2E8F0"}`, background:a?"#FF6B0022":"transparent", color:a?"#C2410C":"#64748B", fontSize:12, fontWeight:700, cursor:"pointer" });
  ...
  const renderGroupTable = (teamSubset:string[], qCut:number) => {
    ...
    return (
      <table style={{ width:"100%", borderCollapse:"collapse" }}>
        ...
        <tbody>
          {rows.map((t,i)=>(
            <tr key={t.team} style={{ borderBottom:"1px solid #E2E8F0", background:i<qCut?"rgba(255,107,0,.04)":"transparent", position:"relative" }}>
              <td style={{ padding:"12px 10px", fontSize:13, fontWeight:900, color:i===0?"#C2410C":i===1?"#C0C0C0":i===2?"#CD7F32":"#64748B" }}>{i<3?["🥇","🥈","🥉"][i]:i+1}</td>
              <td style={{ padding:"12px 10px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <div style={{ width:8, height:8, borderRadius:"50%", background:TEAM_COLORS[t.team]||"#64748B", flexShrink:0 }}/>
                  <span style={{ fontSize:13, fontWeight:700, color:"#1E293B" }}>{t.team}</span>
                </div>
              </td>
              <td style={{ padding:"12px 10px", fontSize:12, color:"#64748B" }}>{t.p}</td>
              <td style={{ padding:"12px 10px", fontSize:12, fontWeight:700, color:"#047857" }}>{t.w}</td>
              <td style={{ padding:"12px 10px", fontSize:12, color:"#DC2626" }}>{t.l}</td>
              <td style={{ padding:"12px 10px", fontSize:12, color:"#64748B" }}>{t.nr}</td>
              <td style={{ padding:"12px 10px", fontSize:16, fontWeight:900, color:"#C2410C" }}>{t.pts}</td>
              <td style={{ padding:"12px 10px", fontSize:12, fontWeight:700, color:t.nrr.startsWith("+")?"#047857":"#EF4444" }}>{t.nrr}</td>
              <td style={{ padding:"12px 10px" }}>
                <div style={{ display:"flex", gap:3 }}>
                  {t.form.slice(-5).map((r,j)=>(
                    <div key={j} style={{ width:18, height:18, borderRadius:4, background:r==="W"?"#10B98130":r==="N"?"#F59E0B30":"#EF444430", display:"flex", alignItems:"center", justifyContent:"center", fontSize:8, fontWeight:800, color:r==="W"?"#047857":r==="N"?"#F59E0B":"#EF4444" }}>{r}</div>
                  ))}
                </div>
              </td>
            </tr>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800, color:"#1E293B" }}>Leaderboard</div>
          <div style={{ fontSize:12, color:"#64748B", marginTop:2 }}>Season 5 · batting, bowling, points table, and groups</div>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button style={{ padding:"9px 16px", borderRadius:9, border:"1px solid #E2E8F0", background:"transparent", color:"#64748B", fontSize:12, cursor:"pointer" }}>🔗 Embed on Website</button>
      ...
      {tab==="batting"&&(
        <div style={card}>
          <div style={{ fontSize:14, fontWeight:700, color:"#1E293B", marginBottom:16 }}>Most Runs — Season 5</div>
          {batters.map((p,i)=>(
            <div key={i} style={{ display:"flex", alignItems:"center", gap:14, padding:"12px 0", borderBottom:"1px solid #E2E8F0" }}>
              <div style={{ width:28, height:28, borderRadius:8, background:i<3?"#FF6B0020":"#E2E8F0", border:`1px solid ${i<3?"#FF6B0040":"#E2E8F0"}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:i<3?"#C2410C":"#475569", flexShrink:0 }}>{i+1}</div>
              ...
              <div style={{ textAlign:"center", minWidth:50 }}>
                <div style={{ fontSize:20, fontWeight:900, color:"#C2410C" }}>{p.runs}</div>
                <div style={{ fontSize:9, color:"#475569" }}>RUNS</div>
              </div>
              <div style={{ textAlign:"center", minWidth:40 }}>
                <div style={{ fontSize:14, fontWeight:700, color:"#047857" }}>{p.avg}</div>
                <div style={{ fontSize:9, color:"#475569" }}>AVG</div>
              </div>
              <div style={{ textAlign:"center", minWidth:40 }}>
                <div style={{ fontSize:14, fontWeight:700, color:"#4F46E5" }}>{p.sr}</div>
                <div style={{ fontSize:9, color:"#475569" }}>SR</div>
              </div>
              <div style={{ textAlign:"center", minWidth:40 }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#1E293B" }}>{p.hs}</div>
                <div style={{ fontSize:9, color:"#475569" }}>HS</div>
              </div>
              <div style={{ textAlign:"center", minWidth:40 }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#64748B" }}>{p.fours}/{p.sixes}</div>
      ...
      {tab==="bowling"&&(
        <div style={card}>
          <div style={{ fontSize:14, fontWeight:700, color:"#1E293B", marginBottom:16 }}>Most Wickets — Season 5</div>
          {bowlers.map((p,i)=>(
            <div key={i} style={{ display:"flex", alignItems:"center", gap:14, padding:"12px 0", borderBottom:"1px solid #E2E8F0" }}>
              <div style={{ width:28, height:28, borderRadius:8, background:i<3?"#FF6B0020":"#E2E8F0", border:`1px solid ${i<3?"#FF6B0040":"#E2E8F0"}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:i<3?"#C2410C":"#475569", flexShrink:0 }}>{i+1}</div>
              <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ textAlign:"center", minWidth:50 }}>
                <div style={{ fontSize:20, fontWeight:900, color:"#DC2626" }}>{p.wickets}</div>
                <div style={{ fontSize:9, color:"#475569" }}>WKTS</div>
              </div>
              <div style={{ textAlign:"center", minWidth:40 }}>
                <div style={{ fontSize:14, fontWeight:700, color:p.economy<=7?"#047857":"#EF4444" }}>{p.economy}</div>
                <div style={{ fontSize:9, color:"#475569" }}>ECON</div>
              </div>
              <div style={{ textAlign:"center", minWidth:40 }}>
                <div style={{ fontSize:13, fontWeight:700, color:"#1E293B" }}>{p.matches}</div>
      ...
      {tab==="points"&&(
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
            {!editMode
              ? <button onClick={startEdit} style={{ padding:"9px 18px", borderRadius:9, border:"1px solid #FF6B0040", background:"#FF6B0012", color:"#C2410C", fontSize:12, fontWeight:700, cursor:"pointer" }}>✏️ Edit Table</button>
              : <>
                  ...
                  <button onClick={()=>setEditMode(false)} style={{ padding:"9px 16px", borderRadius:9, border:"1px solid #E2E8F0", background:"transparent", color:"#64748B", fontSize:12, cursor:"pointer" }}>Cancel</button>
                </>
            }
            {/* Quick result entry */}
          ...
          {editMode&&(
            <div style={card}>
              <div style={{ fontSize:13, fontWeight:700, color:"#1E293B", marginBottom:14 }}>✏️ Edit Mode — modify any field and click Save</div>
                        ...
                        <tr key={i} style={{ borderBottom:"1px solid #E2E8F0" }}>
                          <td style={{ padding:"8px 10px" }}>
                            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                              ...
                              <span style={{ fontSize:12, fontWeight:700, color:"#1E293B" }}>{r.team}</span>
                            </div>
                          </td>
                          {(["p","w","l","nr"] as const).map(f=>(
                            <td key={f} style={{ padding:"6px 8px" }}>
                              <input type="number" value={r[f] as number} onChange={e=>editCell(f,e.target.value)} min={0}
                                style={{ width:50, padding:"5px 8px", background:"#F5F6F8", border:"1px solid #E2E8F0", borderRadius:6, color:"#1E293B", fontSize:12, outline:"none", textAlign:"center" }}/>
                            </td>
                          ))}
                          <td style={{ padding:"6px 8px" }}>
                            <input value={r.nrr} onChange={e=>editCell("nrr",e.target.value)}
                              style={{ width:70, padding:"5px 8px", background:"#F5F6F8", border:"1px solid #E2E8F0", borderRadius:6, color:"#1E293B", fontSize:12, outline:"none" }}/>
                          </td>
                          <td style={{ padding:"6px 8px" }}>
                            <div style={{ display:"flex", gap:3 }}>
                              {r.form.slice(-5).map((f2,j)=>(
                                <select key={j} value={f2} onChange={e=>{ const nf=[...r.form]; nf[nf.length-5+j]=e.target.value; updateRow(i,"form",nf as any); }}
                                  style={{ width:38, padding:"3px 4px", background:"#F5F6F8", border:"1px solid #E2E8F0", borderRadius:4, color:f2==="W"?"#047857":f2==="N"?"#F59E0B":"#EF4444", fontSize:10, outline:"none" }}>
                              ...
                              <button onClick={()=>{ const nf=[...r.form,"W"].slice(-5); updateRow(i,"form",nf as any); updateRow(i,"w",r.w+1); updateRow(i,"p",r.p+1); }}
                                style={{ padding:"2px 6px", borderRadius:4, border:"1px solid #10B98130", background:"#10B98110", color:"#047857", fontSize:9, cursor:"pointer", fontWeight:700 }}>+W</button>
                              <button onClick={()=>{ const nf=[...r.form,"L"].slice(-5); updateRow(i,"form",nf as any); updateRow(i,"l",r.l+1); updateRow(i,"p",r.p+1); }}
                                style={{ padding:"2px 6px", borderRadius:4, border:"1px solid #EF444430", background:"#EF444410", color:"#DC2626", fontSize:9, cursor:"pointer", fontWeight:700 }}>+L</button>
                              <button onClick={()=>{ const nf=[...r.form,"N"].slice(-5); updateRow(i,"form",nf as any); updateRow(i,"nr",r.nr+1); updateRow(i,"p",r.p+1); }}
                                style={{ padding:"2px 6px", borderRadius:4, border:"1px solid #F59E0B30", background:"#F59E0B10", color:"#B45309", fontSize:9, cursor:"pointer", fontWeight:700 }}>+NR</button>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          {!editMode&&(
            <div style={card}>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                ...
                <tbody>
                  {tableData.map((t,i)=>(
                    <tr key={t.team} style={{ borderBottom:"1px solid #E2E8F0", background:i<4?"rgba(255,107,0,.03)":"transparent" }}>
                      <td style={{ padding:"13px 12px", fontSize:14, fontWeight:900, color:i===0?"#B45309":i===1?"#C0C0C0":i===2?"#CD7F32":i<4?"#FF6B00":"#64748B" }}>
                        {i<3?["🥇","🥈","🥉"][i]:i+1}
                      </td>
                      ...
                      <td style={{ padding:"13px 12px", fontSize:13, color:"#64748B" }}>{t.p}</td>
                      <td style={{ padding:"13px 12px", fontSize:13, fontWeight:700, color:"#047857" }}>{t.w}</td>
                      <td style={{ padding:"13px 12px", fontSize:13, color:"#DC2626" }}>{t.l}</td>
                      <td style={{ padding:"13px 12px", fontSize:13, color:"#64748B" }}>{t.nr}</td>
                      <td style={{ padding:"13px 12px", fontSize:16, fontWeight:900, color:"#C2410C" }}>{t.pts}</td>
                      <td style={{ padding:"13px 12px", fontSize:13, fontWeight:700, color:t.nrr.startsWith("+")?`#047857`:"#EF4444" }}>{t.nrr}</td>
                      <td style={{ padding:"13px 12px" }}>
                        <div style={{ display:"flex", gap:4 }}>
                          {t.form.slice(-5).map((r,j)=>(
                            <div key={j} style={{ width:20, height:20, borderRadius:5, background:r==="W"?"#10B98130":r==="N"?"#F59E0B30":"#EF444430", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:800, color:r==="W"?"#047857":r==="N"?"#F59E0B":"#EF4444" }}>{r}</div>
                          ))}
                        </div>
                      </td>
                    </tr>
      ...
      {tab==="groups"&&(
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {/* Create group */}
          <div style={card}>
            <div style={{ fontSize:14, fontWeight:700, color:"#1E293B", marginBottom:14 }}>⬡ Group Management</div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <input value={newGName} onChange={e=>setNewGName(e.target.value)}
                ...
                style={{ flex:1, padding:"9px 12px", background:"#F5F6F8", border:"1px solid #E2E8F0", borderRadius:8, color:"#1E293B", fontSize:13, outline:"none" }}/>
          ...
          {groups.map(g=>(
            <div key={g.id} style={card}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
                <div>
                  <div style={{ fontSize:15, fontWeight:800, color:"#C2410C" }}>⬡ {g.name}</div>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button onClick={()=>setAssigning(assigning===g.id?null:g.id)}
                    style={{ padding:"7px 14px", borderRadius:8, border:"1px solid #FF6B0040", background:"#FF6B0012", color:"#C2410C", fontSize:11, fontWeight:700, cursor:"pointer" }}>
                    {assigning===g.id?"Done ✓":"Assign Teams"}
                  </button>
                  <button onClick={()=>removeGroup(g.id)}
                    style={{ padding:"7px 12px", borderRadius:8, border:"1px solid #EF444430", background:"#EF444410", color:"#DC2626", fontSize:11, cursor:"pointer", fontWeight:700 }}>
                    Delete
                  </button>
                </div>
              </div>
                    ...
                    {ALL_TEAMS_LIST.map(team=>{
                      ...
                      return (
                        <button key={team} onClick={()=>toggleTeamInGroup(g.id,team)}
                          style={{ padding:"6px 14px", borderRadius:20, border:`1px solid ${inGroup?(TEAM_COLORS[team]||"#FF6B00"):"#E2E8F0"}`, background:inGroup?`${TEAM_COLORS[team]||"#FF6B00"}18`:"transparent", color:inGroup?(TEAM_COLORS[team]||"#C2410C"):"#64748B", fontSize:11, fontWeight:700, cursor:"pointer", transition:"all .15s" }}>
                          {inGroup?"✓ ":""}{team}
                        </button>
                      );
                    })}
              ...
              {g.teams.length>0&&(
                <>
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:g.teams.length>=2?16:0 }}>
                    {g.teams.map(t=>(
                      <div key={t} style={{ padding:"4px 12px", borderRadius:20, background:`${TEAM_COLORS[t]||"#64748B"}18`, border:`1px solid ${TEAM_COLORS[t]||"#64748B"}40`, fontSize:11, fontWeight:700, color:TEAM_COLORS[t]||"#1E293B" }}>
                        {t}
                      </div>
                    ))}
                  </div>
