  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800, color:"#1E293B" }}>Support Tickets</div>
          <div style={{ fontSize:12, color:"#64748B", marginTop:2 }}>Manage player queries — payment, KYC, trial, technical issues</div>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <span style={{ fontSize:12, fontWeight:700, color:"#DC2626", background:"#EF444415", padding:"6px 12px", borderRadius:8, border:"1px solid #EF444430" }}>{open} Open</span>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
        {[
          ...
          { label:"Resolved Today",   value:TICKETS.filter(t=>t.status==="Resolved").length, color:"#047857" },
        ].map(s=>(
      ...
      <div style={{ display:"grid", gridTemplateColumns: sel?"360px 1fr":"1fr", gap:16 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {categories.map(c=>(
              <button key={c} onClick={()=>setFilter(c)} style={{ padding:"6px 14px", borderRadius:8, border:`1px solid ${filter===c?"#FF6B00":"#E2E8F0"}`, background:filter===c?"#FF6B0022":"transparent", color:filter===c?"#C2410C":"#64748B", fontSize:11, fontWeight:700, cursor:"pointer" }}>{c}</button>
            ))}
          </div>
          {filtered.length===0 && (
            <div style={{ ...card, padding:"36px 16px", textAlign:"center", color:"#64748B", fontSize:12 }}>
          ...
          {filtered.map(t=>{
            ...
            return (
              <div key={t.id} onClick={()=>setSel(s=>s?.id===t.id?null:t)} style={{ ...card, cursor:"pointer", border:`1px solid ${sel?.id===t.id?"#FF6B0060":"#E2E8F0"}`, padding:16 }}>
                ...
                <div style={{ fontSize:13, fontWeight:600, color:"#1E293B", marginBottom:5 }}>{t.issue}</div>
                <div style={{ fontSize:11, color:"#64748B" }}>{t.player} · {t.city} · {t.created}</div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:8 }}>
                  <span style={{ fontSize:10, color:"#64748B" }}>Category: {t.category}</span>
                  <span style={{ fontSize:10, fontWeight:700, color:slaOk?"#047857":"#EF4444" }}>SLA: {slaOk?"✓ On time":"⚠ Breached"}</span>
                </div>
              </div>
            );
          })}
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={card}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                <div>
                  <div style={{ fontSize:14, fontWeight:700, color:"#1E293B" }}>{sel.issue}</div>
                  <div style={{ fontSize:11, color:"#64748B", marginTop:3 }}>{sel.player} · {sel.city} · {sel.id}</div>
                </div>
                <div style={{ display:"flex", gap:8 }}>
                  <button style={{ padding:"6px 14px", borderRadius:8, border:"none", background:"#10B98120", color:"#047857", fontSize:11, fontWeight:700, cursor:"pointer" }}>✓ Resolve</button>
                  <button style={{ padding:"6px 14px", borderRadius:8, border:"none", background:"#EF444420", color:"#DC2626", fontSize:11, cursor:"pointer" }}>✗ Close</button>
                </div>
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:12, maxHeight:300, overflowY:"auto" }}>
                {sel.messages.map((m,i)=>(
                  <div key={i} style={{ display:"flex", justifyContent:m.sender==="agent"?"flex-end":"flex-start" }}>
                    <div style={{ maxWidth:"75%", padding:"10px 14px", borderRadius:12, background:m.sender==="agent"?"#FF6B0022":"#E2E8F0", borderBottomRightRadius:m.sender==="agent"?4:12, borderBottomLeftRadius:m.sender==="agent"?12:4 }}>
                      <div style={{ fontSize:12, color:"#1E293B", lineHeight:1.5 }}>{m.text}</div>
              ...
              <div style={{ marginTop:16, display:"flex", gap:10, alignItems:"flex-end" }}>
                <textarea value={reply} onChange={e=>setReply(e.target.value)} placeholder="Type your reply…" rows={3}
                  style={{ flex:1, padding:"10px 12px", borderRadius:9, border:"1px solid #E2E8F0", background:"#F5F6F8", color:"#1E293B", fontSize:12, outline:"none", resize:"none", lineHeight:1.5 }}/>
              ...
              <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
                {["Payment confirmed","KYC approved","Slot confirmed","Will resolve in 24h"].map(t=>(
                  <button key={t} onClick={()=>setReply(t)} style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #E2E8F0", background:"transparent", color:"#64748B", fontSize:10, cursor:"pointer" }}>{t}</button>
                ))}
              </div>
            </div>
          </div>
