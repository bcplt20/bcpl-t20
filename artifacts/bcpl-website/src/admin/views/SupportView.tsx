    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          ...
          <div style={{ fontSize:12, color:"#A6B3D0", marginTop:2 }}>Manage player queries — payment, KYC, trial, technical issues</div>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns: sel?"360px 1fr":"1fr", gap:16 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {categories.map(c=>(
              <button key={c} onClick={()=>setFilter(c)} style={{ padding:"6px 14px", borderRadius:8, border:`1px solid ${filter===c?"#FF6B00":"#33436B"}`, background:filter===c?"#FF6B0022":"transparent", color:filter===c?"#FF6B00":"#A6B3D0", fontSize:11, fontWeight:700, cursor:"pointer" }}>{c}</button>
            ))}
          </div>
          {filtered.length===0 && (
            <div style={{ ...card, padding:"36px 16px", textAlign:"center", color:"#8593B3", fontSize:12 }}>
          ...
          {filtered.map(t=>{
            ...
            return (
              <div key={t.id} onClick={()=>setSel(s=>s?.id===t.id?null:t)} style={{ ...card, cursor:"pointer", border:`1px solid ${sel?.id===t.id?"#FF6B0060":"#33436B"}`, padding:16 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                  <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                    <span style={{ fontSize:10, fontWeight:800, color:"#94A3C4" }}>{t.id}</span>
                ...
                <div style={{ fontSize:11, color:"#A6B3D0" }}>{t.player} · {t.city} · {t.created}</div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:8 }}>
                  <span style={{ fontSize:10, color:"#8593B3" }}>Category: {t.category}</span>
            ...
            <div style={card}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:"#A6B3D0", marginTop:3 }}>{sel.player} · {sel.city} · {sel.id}</div>
                </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12, maxHeight:300, overflowY:"auto" }}>
                {sel.messages.map((m,i)=>(
                  <div key={i} style={{ display:"flex", justifyContent:m.sender==="agent"?"flex-end":"flex-start" }}>
                    <div style={{ maxWidth:"75%", padding:"10px 14px", borderRadius:12, background:m.sender==="agent"?"#FF6B0022":"#33436B", borderBottomRightRadius:m.sender==="agent"?4:12, borderBottomLeftRadius:m.sender==="agent"?12:4 }}>
                      <div style={{ fontSize:12, color:"#F1F5F9", lineHeight:1.5 }}>{m.text}</div>
                      <div style={{ fontSize:9, color:"#94A3C4", marginTop:5 }}>{m.sender==="agent"?"Support · ":""}{m.time}</div>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{ marginTop:16, display:"flex", gap:10, alignItems:"flex-end" }}>
                <textarea value={reply} onChange={e=>setReply(e.target.value)} placeholder="Type your reply…" rows={3}
                  style={{ flex:1, padding:"10px 12px", borderRadius:9, border:"1px solid #33436B", background:"#243050", color:"#E2E8F0", fontSize:12, outline:"none", resize:"none", lineHeight:1.5 }}/>
              ...
              <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
                {["Payment confirmed","KYC approved","Slot confirmed","Will resolve in 24h"].map(t=>(
                  <button key={t} onClick={()=>setReply(t)} style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", fontSize:10, cursor:"pointer" }}>{t}</button>
                ))}
              </div>
            </div>
          </div>
