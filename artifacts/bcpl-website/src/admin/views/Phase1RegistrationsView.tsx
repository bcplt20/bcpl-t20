...
export default function Phase1RegistrationsView({ onNavigate, focusId, initialFilter, refreshTick = 0 }: { onNavigate?: (tab: string, payload?: NavPayload) => void; focusId?: string; initialFilter?: string; refreshTick?: number }) {
  ...
  const statCards = stats?.registrations ? [
    { label:"Total Registrations", value: stats.registrations.total,         color:"#4F46E5" },
    { label:"Payment Done",        value: stats.registrations.paymentDone,   color:"#1D4ED8" },
    { label:"Video Submitted",     value: stats.registrations.videoSubmitted, color:"#7C3AED" },
    { label:"Selected",            value: stats.registrations.selected,       color:"#047857" },
    { label:"Rejected",            value: stats.registrations.rejected,       color:"#DC2626" },
  ] : [];
  ...
  return (
    ...
    <div style={{ flex:1, minWidth:0, display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:10 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800, color:"#1E293B" }}>Phase 1 Registrations</div>
          <div style={{ fontSize:12, color:"#64748B", marginTop:2 }}>
            Manage all Phase 1 registrations — review payments, videos and selection decisions
          </div>
        </div>
        <button onClick={() => load()} style={{ padding:"9px 16px", borderRadius:9, border:"1px solid #E2E8F0", background:"transparent", color:"#64748B", fontSize:12, fontWeight:700, cursor:"pointer" }}>
          ↺ Refresh
        </button>
      </div>
      ...
      {/* Error */}
      {err && (
        <div style={{ padding:14, borderRadius:10, background:"#EF444415", border:"1px solid #EF444440", color:"#DC2626", fontSize:12 }}>
          ⚠ {err} — <button onClick={() => load()} style={{ background:"none", border:"none", color:"#DC2626", cursor:"pointer", fontWeight:700, fontSize:12 }}>Retry</button>
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:4 }}>
          {["all", "pending", "payment_done", "video_submitted", "selected", "rejected"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding:"7px 13px", borderRadius:8, border:`1px solid ${filter===f?"#FF6B00":"#E2E8F0"}`, background:filter===f?"#FF6B0022":"transparent", color:filter===f?"#C2410C":"#64748B", fontSize:11, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap" }}>
        ...
        <select value={roleF} onChange={e => setRoleF(e.target.value)}
          style={{ padding:"8px 10px", borderRadius:8, border:"1px solid #E2E8F0", background:"#FFFFFF", color:"#64748B", fontSize:12, cursor:"pointer", outline:"none" }}>
        ...
        <input value={cityQ} onChange={e => setCityQ(e.target.value)} onKeyDown={e => e.key === "Enter" && load()} placeholder="Filter by city…"
          style={{ padding:"8px 12px", borderRadius:8, border:"1px solid #E2E8F0", background:"#FFFFFF", color:"#1E293B", fontSize:12, outline:"none", width:140 }} />
        {/* Name/phone search */}
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name / phone…"
          style={{ padding:"8px 12px", borderRadius:8, border:"1px solid #E2E8F0", background:"#FFFFFF", color:"#1E293B", fontSize:12, outline:"none", width:180 }} />
      </div>

      {/* Table */}
      <div style={{ ...card, padding:0, overflow:"hidden" }}>
        {loading ? (
          <div style={{ padding:60, textAlign:"center", color:"#64748B", fontSize:14 }}>Loading registrations…</div>
        ) : displayed.length === 0 ? (
              ...
              <thead>
                <tr style={{ background:"#FFFFFF", borderBottom:"1px solid #E2E8F0" }}>
                  {["Reg ID","Player","Contact","City","Role","Reg Date","Payment","Video","Phase 1 Status","Actions"].map(h => (
                    <th key={h} style={{ padding:"10px 12px", textAlign:"left", fontSize:10, fontWeight:700, color:"#64748B", letterSpacing:"0.06em", textTransform:"uppercase", whiteSpace:"nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {displayed.map((r, i) => (
                  <tr key={r.id} onClick={() => setDetail(r)} title="Tap for full details"
                    style={{ borderBottom:"1px solid #E2E8F0", cursor:"pointer", background:detail?.id===r.id?"#FF6B0012":(i%2===0?"transparent":"#FFFFFF") }}>
                    {/* Reg ID */}
                    ...
                    {/* Player */}
                    <td style={{ padding:"10px 12px" }}>
                      <div style={{ fontWeight:700, color:"#1E293B" }}>{r.user?.name ?? "—"}</div>
                    </td>
                    {/* Contact */}
                    <td style={{ padding:"10px 12px" }}>
                      <div style={{ color:"#64748B" }}>{r.user?.phone ?? "—"}</div>
                      <div style={{ fontSize:10, color:"#475569" }}>{r.user?.email ?? "—"}</div>
                    </td>
                    {/* City */}
                    <td style={{ padding:"10px 12px", color:"#64748B", whiteSpace:"nowrap" }}>{r.trialCity ?? "—"}</td>
                    {/* Role */}
                    <td style={{ padding:"10px 12px", color:"#64748B", whiteSpace:"nowrap" }}>{ROLE_LABEL[r.role] ?? r.role}</td>
                    {/* Reg Date */}
                    ...
                    <td style={{ padding:"10px 12px" }}>
                      ...
                      ) : <span style={{ color:"#64748B", fontSize:11 }}>No payment</span>}
                    </td>
                    {/* Video */}
                    <td style={{ padding:"10px 12px" }}>
                      {r.video ? (
                        <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
                          ...
                          {r.video.s3Url && (
                            <a href={r.video.s3Url} target="_blank" rel="noreferrer" title="Watch video in new tab" onClick={e => e.stopPropagation()}
                              style={{ fontSize:10, fontWeight:700, color:"#4F46E5", textDecoration:"none", padding:"3px 8px", borderRadius:5, background:"#6366F118", border:"1px solid #6366F140", display:"inline-block", width:"fit-content" }}>▶ Watch</a>
                          )}
                        </div>
                      ) : <span style={{ color:"#64748B", fontSize:11 }}>No video</span>}
                    </td>
                    {/* Phase 1 Status */}
                    <td style={{ padding:"10px 12px" }}>
                      ...
                      <div style={{ display:"flex", gap:5 }}>
                        {r.phase1Status !== "selected" && (
                          <button
                            ...
                            style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #10B98140", background:"#10B98115", color:"#047857", fontSize:10, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap", opacity:acting===r.id?0.5:1 }}>
                            ✓ Select
                          </button>
                        )}
                        {r.phase1Status !== "rejected" && (
                          <button
                            ...
                            style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #EF444440", background:"#EF444415", color:"#DC2626", fontSize:10, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap", opacity:acting===r.id?0.5:1 }}>
                            ✗ Reject
                          </button>
                        )}
    ...
    {detail && (() => {
      ...
      const row = (label: string, value: React.ReactNode) => (
        <div key={label} style={{ display:"flex", justifyContent:"space-between", gap:10, padding:"7px 0", borderBottom:"1px solid #E2E8F0" }}>
          <span style={{ fontSize:11, color:"#475569", flexShrink:0 }}>{label}</span>
          <span style={{ fontSize:12, color:"#1E293B", textAlign:"right", wordBreak:"break-word" }}>{value}</span>
        </div>
      );
      return (
        <aside style={{ width:320, flexShrink:0, display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ ...card, position:"relative" }}>
            ...
            <div style={{ fontSize:16, fontWeight:800, color:"#1E293B", paddingRight:24 }}>{detail.user?.name ?? "Unknown player"}</div>
            <div style={{ fontSize:11, color:"#64748B", marginTop:4 }}>{ROLE_LABEL[detail.role] ?? detail.role} · {detail.trialCity || "—"}</div>
            <div style={{ marginTop:10, padding:"8px 10px", background:"#FFFFFF", border:"1px solid #E2E8F0", borderRadius:8 }}>
              ...
              <div style={{ fontSize:9, color:"#64748B", fontFamily:"monospace", wordBreak:"break-all", marginTop:6 }}>Internal: {detail.id}</div>
            </div>
            <div style={{ marginTop:10 }}>
              {row("Phone", detail.user?.phone ?? "—")}
              {row("Email", detail.user?.email ? <a href={`mailto:${detail.user.email}`} style={{ color:"#4F46E5" }}>{detail.user.email}</a> : "—")}
              {row("Phase 1", <Badge status={detail.phase1Status} map={P1_STATUS_LABEL} colorMap={P1_STATUS_COLOR} />)}
              {row("Phase 2", detail.phase2Status ? <Badge status={detail.phase2Status} map={{ payment_done:"Payment Done", kyc_done:"KYC Done", selected:"Selected", rejected:"Rejected" }} colorMap={{ payment_done:"#3B82F6", kyc_done:"#A855F7", selected:"#10B981", rejected:"#EF4444" }} /> : <span style={{ color:"#64748B" }}>Not started</span>)}
              ...
              {row("Video", detail.video ? (detail.video.s3Url ? <a href={detail.video.s3Url} target="_blank" rel="noreferrer" style={{ color:"#4F46E5" }}>▶ Watch</a> : detail.video.status) : "No video")}
              {row("KYC", detail.kyc ? <Badge status={detail.kyc.status} map={{ pending:"Pending", verified:"Verified", failed:"Failed" }} colorMap={{ pending:"#F59E0B", verified:"#10B981", failed:"#EF4444" }} /> : <span style={{ color:"#64748B" }}>Not submitted</span>)}
            </div>
            {detail.kyc && (
              <button onClick={() => onNavigate?.("phase2_kyc")}
                style={{ width:"100%", marginTop:12, padding:"8px 0", borderRadius:8, border:"1px solid #A855F744", background:"#A855F715", color:"#7C3AED", fontSize:11, fontWeight:700, cursor:"pointer" }}>
                Open in Phase 2 · KYC
              </button>
            )}
          </div>
            ...
            {journey.map((s, i) => (
              <div key={s.label} style={{ display:"flex", gap:10, alignItems:"stretch" }}>
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flexShrink:0 }}>
                  <div style={{ width:10, height:10, borderRadius:"50%", background:s.done?"#10B981":"#E2E8F0", border:`2px solid ${s.done?"#10B981":"#E2E8F0"}`, marginTop:2 }}/>
                  {i<journey.length-1 && <div style={{ width:2, flex:1, minHeight:14, background:s.done?"#10B98144":"#E2E8F0", marginTop:2, marginBottom:2 }}/>}
                </div>
                <div style={{ paddingBottom:i<journey.length-1?12:0 }}>
                  <div style={{ fontSize:12, fontWeight:600, color:s.done?"#1E293B":"#475569" }}>{s.label}</div>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {detail.phase1Status !== "selected" && (
                <button disabled={acting===detail.id} onClick={() => setPhase1Status(detail.id, "selected")}
                  style={{ flex:1, padding:"8px 0", borderRadius:8, border:"1px solid #10B98140", background:"#10B98115", color:"#047857", fontSize:11, fontWeight:700, cursor:"pointer", opacity:acting===detail.id?0.5:1 }}>✓ Select</button>
              )}
              {detail.phase1Status !== "rejected" && (
                <button disabled={acting===detail.id} onClick={() => setPhase1Status(detail.id, "rejected")}
                  style={{ flex:1, padding:"8px 0", borderRadius:8, border:"1px solid #EF444440", background:"#EF444415", color:"#DC2626", fontSize:11, fontWeight:700, cursor:"pointer", opacity:acting===detail.id?0.5:1 }}>✗ Reject</button>
              )}
