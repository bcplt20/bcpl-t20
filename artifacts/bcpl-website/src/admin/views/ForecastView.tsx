...
export default function ForecastView() {
  ...
  const projection = useMemo(() => {
    ...
    const scenarios = [
      { name: "Slower (×0.7)", mult: 0.7, color: "#C3CEE3" },
  ...
  if (!data) {
    return (
      <div style={{ ...card, textAlign: "center", color: "#A6B3D0", fontSize: 13 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        {[
          ...
          <div key={s.label} style={{ ...card, padding: 16 }}>
            ...
            <div style={{ fontSize: 11, color: "#C3CEE3", marginTop: 4, fontWeight: 700 }}>{s.label}</div>
            <div style={{ fontSize: 10, color: "#94A3C4", marginTop: 2 }}>{s.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={card}>
        <div style={{ display: "grid", gridTemplateColumns: "160px 200px 1fr", gap: 16, alignItems: "end" }}>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#C3CEE3", marginBottom: 6 }}>
      ...
      <div style={card}>
        ...
        {rows.length === 0 ? (
          <div style={{ fontSize: 12, color: "#94A3C4" }}>No registrations yet — data appears here as players sign up.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ display: "grid", gridTemplateColumns: "90px 1fr 70px 70px 110px 90px", gap: 10, fontSize: 10, fontWeight: 800, color: "#94A3C4" }}>
            ...
            {rows.map(r => {
              ...
              return (
                <div key={r.month} style={{ display: "grid", gridTemplateColumns: "90px 1fr 70px 70px 110px 90px", gap: 10, alignItems: "center" }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3" }}>{monthLabel(r.month)}</span>
                  <div style={{ height: 14, borderRadius: 7, background: "#33436B", overflow: "hidden", position: "relative" }}>
      ...
      {projection && (
        <div style={card}>
          ...
          <div style={{ fontSize: 11, color: "#94A3C4", marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
            {projection.scenarios.map(s => (
              <div key={s.name} style={{ border: `1px solid ${s.color}44`, borderRadius: 12, padding: 16, background: `${s.color}0A` }}>
                ...
                <div style={{ fontSize: 11, color: "#C3CEE3", marginTop: 2 }}>registrations · {fmtINR(s.revenue)} revenue</div>
