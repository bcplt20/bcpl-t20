...
export default function MatchesView({ onOpenScoring }: { onOpenScoring?: () => void }) {
  ...
  const inputStyle: React.CSSProperties = { width: "100%", padding: "10px 12px", background: "#F5F6F8", border: "1px solid #E2E8F0", borderRadius: 9, color: "#1E293B", fontSize: 13, outline: "none", boxSizing: "border-box" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#1E293B" }}>Match Management</div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => { setLoading(true); reload(); }} title="Refresh"
            style={{ padding: "10px 14px", borderRadius: 10, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
            ⟳ Refresh
          </button>
          <button onClick={() => setShowBulk(true)} style={{ padding: "10px 18px", borderRadius: 10, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
            📊 Bulk Upload via CSV
          </button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        {[
          { label: "Total Matches", value: matches.length,                                    color: "#4F46E5", icon: "🏏" },
          { label: "Live Now",      value: matches.filter(m => grp(m.status) === "live").length,      color: "#DC2626", icon: "🔴" },
          { label: "Scheduled",     value: matches.filter(m => grp(m.status) === "scheduled").length, color: "#1D4ED8", icon: "📅" },
          { label: "Completed",     value: matches.filter(m => grp(m.status) === "completed").length, color: "#047857", icon: "✅" },
        ].map(s => (
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {(["all", "live", "scheduled", "completed"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            borderColor: filter === f ? groupColor(f === "all" ? "scheduled" : f) : "#E2E8F0",
      ...
      {!loading && matches.length === 0 && !loadErr && (
        <div style={{ ...card, textAlign: "center", padding: 40 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: "#1E293B" }}>No matches scheduled yet</div>
          ...
          <div key={m.id} style={{ ...card, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              <div style={{ flex: 1, textAlign: "center" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: m.winner === m.team1 ? "#047857" : "#F1F5F9", marginBottom: 6 }}>{m.team1}</div>
              </div>
              <div style={{ textAlign: "center", minWidth: 170 }}>
                ...
                <div style={{ fontSize: 20, color: "#64748B", fontWeight: 800 }}>VS</div>
                ...
                <div style={{ fontSize: 10, color: "#64748B", marginTop: 2 }}>📍 {m.venue}</div>
                {m.tossWinner && <div style={{ fontSize: 10, color: "#B45309", marginTop: 4 }}>🪙 {m.tossWinner} won toss, chose to {m.tossDecision}</div>}
              </div>
              <div style={{ flex: 1, textAlign: "center" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: m.winner === m.team2 ? "#047857" : "#F1F5F9", marginBottom: 6 }}>{m.team2}</div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 7, marginLeft: 8 }}>
                {grp(m.status) !== "completed" && (
                  <button onClick={() => onOpenScoring?.()} style={{ padding: "7px 14px", borderRadius: 7, border: "none", background: "#EF444422", color: "#DC2626", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                    🔴 Score
                  </button>
                )}
                {grp(m.status) !== "completed" && (
                  <button onClick={() => openResult(m)} style={{ padding: "7px 14px", borderRadius: 7, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 11, cursor: "pointer" }}>
                    🏆 Enter Result
                  </button>
                )}
                {m.status === "scheduled" && (
                  <button disabled={busyId === m.id} onClick={() => cancelMatch(m)} style={{ padding: "7px 14px", borderRadius: 7, border: "none", background: "#EF444415", color: "#DC2626", fontSize: 11, cursor: "pointer", opacity: busyId === m.id ? 0.5 : 1 }}>
                    {busyId === m.id ? "Cancelling…" : "Cancel"}
                  </button>
                )}
                <button disabled={busyId === m.id} onClick={() => deleteMatch(m)} style={{ padding: "7px 14px", borderRadius: 7, border: "1px solid #7F1D1D", background: "transparent", color: "#DC2626", fontSize: 11, cursor: "pointer", opacity: busyId === m.id ? 0.5 : 1 }}>
                  {busyId === m.id ? "Working…" : "Delete"}
                </button>
              </div>
            </div>
            {(m.winner || m.resultDesc) && grp(m.status) === "completed" && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #E2E8F0", display: "flex", gap: 18, flexWrap: "wrap" }}>
                {m.winner && <span style={{ fontSize: 12, color: "#047857", fontWeight: 700 }}>🏆 {m.resultDesc || `Winner: ${m.winner}`}</span>}
                {!m.winner && m.resultDesc && <span style={{ fontSize: 12, color: "#64748B", fontWeight: 600 }}>{m.resultDesc}</span>}
                {m.playerOfMatch && <span style={{ fontSize: 12, color: "#B45309", fontWeight: 600 }}>⭐ Player of the Match: {m.playerOfMatch}</span>}
              </div>
            )}
          </div>
        ))}
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "#00000088", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 20, padding: 28, width: 480 }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#1E293B", marginBottom: 4 }}>🏏 Schedule New Match</div>
            ...
            {addErr && <div style={{ marginTop: 12, padding: "8px 12px", borderRadius: 8, background: "#EF444422", color: "#DC2626", fontSize: 12, fontWeight: 600 }}>⚠ {addErr}</div>}
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={() => setShowAdd(false)} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, cursor: "pointer" }}>Cancel</button>
      ...
      {resultFor && (
        <div style={{ position: "fixed", inset: 0, background: "#00000088", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 20, padding: 28, width: 480 }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#1E293B", marginBottom: 4 }}>🏆 Match Result</div>
            ...
            {resultErr && <div style={{ marginTop: 12, padding: "8px 12px", borderRadius: 8, background: "#EF444422", color: "#DC2626", fontSize: 12, fontWeight: 600 }}>⚠ {resultErr}</div>}
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={() => setResultFor(null)} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, cursor: "pointer" }}>Cancel</button>
      ...
      {showBulk && (
        <div style={{ position: "fixed", inset: 0, background: "#00000088", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 20, padding: 28, width: 640, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#1E293B", marginBottom: 4 }}>📊 Bulk Fixture Upload</div>
            ...
            <div style={{ background: "#F5F6F8", borderRadius: 10, padding: "14px 16px", border: "1px solid #E2E8F0", marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#B45309", marginBottom: 8 }}>📋 Required CSV Format (Date as YYYY-MM-DD, Time as HH:MM):</div>
              ...
              <button onClick={() => { const a = document.createElement("a"); a.href = "data:text/plain," + encodeURIComponent(SAMPLE_CSV); a.download = "bcpl_fixture_template.csv"; a.click(); }} style={{ marginTop: 10, padding: "6px 14px", borderRadius: 7, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 11, cursor: "pointer" }}>
                ⬇ Download Template
              </button>
            </div>
            ...
            <div
              ...
              style={{ background: "#F5F6F8", borderRadius: 12, border: "2px dashed #E2E8F0", padding: "32px", textAlign: "center", cursor: "pointer", marginBottom: 14 }}
              onMouseEnter={e => (e.currentTarget.style.borderColor = "#FF6B00")}
              onMouseLeave={e => (e.currentTarget.style.borderColor = "#E2E8F0")}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>📂</div>
              <div style={{ fontSize: 14, color: "#64748B", fontWeight: 600 }}>Drag & drop your .CSV file here</div>
            ...
            {bulkPreview.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#047857", marginBottom: 10 }}>✓ {bulkPreview.length} matches detected — Preview:</div>
                <div style={{ maxHeight: 180, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
                  {bulkPreview.map((m, i) => (
                    <div key={i} style={{ display: "flex", gap: 12, padding: "10px 12px", background: "#F5F6F8", borderRadius: 8, border: "1px solid #E2E8F0", fontSize: 12, color: "#64748B" }}>
                      <span style={{ color: "#1E293B", fontWeight: 600 }}>{m.team1}</span>
                      <span style={{ color: "#C2410C", fontWeight: 800 }}>vs</span>
                      <span style={{ color: "#1E293B", fontWeight: 600 }}>{m.team2}</span>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => { setShowBulk(false); setBulkPreview([]); setCsvText(""); }} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, cursor: "pointer" }}>Cancel</button>
              <button disabled={bulkPreview.length === 0 || bulkBusy} onClick={importBulk}
                style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "none", background: bulkPreview.length > 0 ? "linear-gradient(135deg,#FF6B00,#FF8C40)" : "#E2E8F0", color: bulkPreview.length > 0 ? "#fff" : "#475569", fontSize: 13, fontWeight: 700, cursor: bulkPreview.length > 0 && !bulkBusy ? "pointer" : "not-allowed" }}>
