...
const kycColor = (k:string) => k==="verified"?"#10B981":k==="failed"?"#EF4444":k==="pending"?"#F59E0B":"#94A3C4";
...
export default function UsersView({ onNavigate, initialQuick, refreshTick = 0 }: { onNavigate?: (tab: string, payload?: NavPayload) => void; initialQuick?: string; refreshTick?: number }) {
  ...
  const stats = [
    ...
    { label:"Not Registered",value:allUsers.filter(u=>!u.registered).length, color:"#A6B3D0", q:"not_registered" as QuickFilter },
  ];
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", gap:14, minWidth:0 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:10 }}>
          {stats.map(s => (
            <div key={s.label} onClick={() => setQuick(s.q)} style={{ ...card, padding:14, cursor:"pointer", borderTop:`3px solid ${s.color}`, opacity:quick===s.q?1:0.65, transition:"all 0.2s" }}>
              ...
              <div style={{ fontSize:10, color:"#A6B3D0", marginTop:3 }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ ...card, padding:"14px 16px" }}>
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
            ...
            <div style={{ position:"relative", flex:"1 1 200px" }}>
              <span style={{ position:"absolute", left:11, top:"50%", transform:"translateY(-50%)", color:"#94A3C4", fontSize:13 }}>🔍</span>
              <input value={search} onChange={e=>setSearch(e.target.value)}
                placeholder="Search name / phone / email…"
                style={{ width:"100%", padding:"9px 12px 9px 34px", background:"#243050", border:"1px solid #33436B", borderRadius:9, color:"#F1F5F9", fontSize:12, outline:"none", boxSizing:"border-box" }} />
            </div>

            {/* State */}
            <select value={state} onChange={e=>{ setState(e.target.value); setCity("All Cities"); }}
              style={{ padding:"9px 12px", background:"#243050", border:"1px solid #33436B", borderRadius:9, color:"#F1F5F9", fontSize:12, outline:"none" }}>
            ...
            <span style={{ fontSize:11, color:"#94A3C4", marginLeft:"auto" }}>{filtered.length} results</span>
            ...
            <button onClick={()=>{
              ...
              const rows=filtered.map(u=>`<tr style="border-bottom:1px solid #E8EDF7"><td>${u.name}</td><td>${u.phone}</td><td>${u.email}</td><td>${u.state}, ${u.city}</td><td>${u.role}</td><td>${u.phase1?"✓ Phase 1":""} ${u.phase2?"✓ Phase 2":""}</td><td>${u.kyc}</td><td>${u.active?"Active":"Inactive"}</td></tr>`).join("");
          ...
          <div style={{ display:"flex", gap:6, marginTop:12, flexWrap:"wrap" }}>
            {(Object.keys(quickLabels) as QuickFilter[]).map(q=>(
              <button key={q} onClick={()=>setQuick(q)} style={{
                ...
                borderColor: quick===q?"#FF6B00":"#33436B",
                background: quick===q?"#FF6B0022":"transparent",
                color: quick===q?"#FF6B00":"#A6B3D0",
        ...
        <div style={{ ...card, padding:0, overflow:"hidden" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ borderBottom:"1px solid #33436B", background:"#243050" }}>
                {["Reg ID","Player","Role","Location","Joined","Phase 1","Phase 2","KYC","Video","Status"].map(h=>(
                  <th key={h} style={{ padding:"11px 14px", textAlign:"left", fontSize:10, color:"#94A3C4", fontWeight:700, textTransform:"uppercase", letterSpacing:.5, whiteSpace:"nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(u=>(
                <tr key={u.id} onClick={()=>setSelected(u)} style={{
                  ...
                  <td style={{ padding:"13px 14px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:10, color:"#94A3C4" }}>{u.phone}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding:"13px 14px" }}>
                  ...
                  <td style={{ padding:"13px 14px" }}>
                    <div style={{ fontSize:12, color:"#C3CEE3" }}>{u.city}</div>
                    <div style={{ fontSize:10, color:"#94A3C4" }}>{u.state}</div>
                  </td>
                  <td style={{ padding:"13px 14px", fontSize:11, color:"#A6B3D0" }}>{u.joined}</td>
              ...
              {filtered.length===0 && (
                <tr><td colSpan={10} style={{ padding:"40px", textAlign:"center", color:"#8593B3", fontSize:13 }}>
      ...
      {selected && (
        <div style={{ width:290, flexShrink:0, display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ ...card, textAlign:"center" }}>
            ...
            <div style={{ marginTop:10 }}>
              ...
              {selected.regNumber
                ...
                : <span style={{ display:"inline-block", padding:"3px 10px", borderRadius:6, fontSize:11, fontWeight:700, background:"#33436B", color:"#A6B3D0", border:"1px solid #3E4E76" }}>Payment pending</span>}
            </div>
            <div onClick={()=>onNavigate?.("phase1_regs",{ focusId:selected.id })} title="Open the full registration record"
              style={{ fontSize:9, color:"#94A3C4", marginTop:6, cursor:"pointer", wordBreak:"break-all", textDecoration:"underline" }}>
          ...
          <div style={{ ...card, padding:16 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#94A3C4", marginBottom:12, textTransform:"uppercase", letterSpacing:.5 }}>Payment Status</div>
            {[{label:"Phase 1 Fee",done:selected.phase1},{label:"Phase 2 Fee",done:selected.phase2}].map(p=>(
              <div key={p.label} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:"1px solid #3A4A72" }}>
                <span style={{ fontSize:12, color:"#C3CEE3" }}>{p.label}</span>
          ...
          <div style={{ ...card, padding:16 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#94A3C4", marginBottom:10, textTransform:"uppercase", letterSpacing:.5 }}>KYC Verification</div>
            ...
            {selected.kycId ? (
              ...
              <div style={{ fontSize:12, color:"#8593B3" }}>KYC not submitted yet</div>
            )}
          </div>
          <div style={{ ...card, padding:16 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#94A3C4", marginBottom:10, textTransform:"uppercase", letterSpacing:.5 }}>Selection Video</div>
            {selected.video && selected.videoUrl
              ? <a href={selected.videoUrl} target="_blank" rel="noreferrer" title="Watch video in new tab"
                  style={{ background:"#243050", borderRadius:10, aspectRatio:"16/9", display:"flex", flexDirection:"column", gap:6, alignItems:"center", justifyContent:"center", cursor:"pointer", border:"1px solid #33436B", textDecoration:"none" }}>
                  <span style={{ fontSize:32 }}>▶️</span>
                  <span style={{ fontSize:11, fontWeight:700, color:"#3B82F6" }}>Watch video</span>
                </a>
              : selected.video
              ? <div style={{ padding:"16px", textAlign:"center", color:"#A6B3D0", fontSize:12, background:"#243050", borderRadius:10, border:"1px solid #33436B" }}>Video submitted — link unavailable</div>
              : <div style={{ padding:"16px", textAlign:"center", color:"#8593B3", fontSize:12, background:"#243050", borderRadius:10, border:"1px dashed #33436B" }}>No video uploaded</div>}
          </div>
        </div>
      )}
    </div>
