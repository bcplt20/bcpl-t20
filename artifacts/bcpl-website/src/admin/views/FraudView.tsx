...
export default function FraudView() {
  ...
  const chip = (active: boolean): React.CSSProperties => ({
    ...
    border: "1px solid " + (active ? "#FF6B00" : "#E2E8F0"),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#1E293B" }}>Fraud Detection</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>
        {[
          { label: "Total Findings",  value: flags.length,          color: "#4F46E5", f: "all" },
          { label: "Needs Review",    value: counts.flagged ?? 0,   color: "#DC2626", f: "flagged" },
          { label: "Cleared",         value: counts.cleared ?? 0,   color: "#047857", f: "cleared" },
          { label: "Blocked",         value: counts.blocked ?? 0,   color: "#7C3AED", f: "blocked" },
        ].map(s => (
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ width: 1, height: 22, background: "#FFFFFF" }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: sel ? "1fr 340px" : "1fr", gap: 12 }}>
        <div style={{ ...card, padding: 0, overflow: "hidden" }}>
          {loading ? (
            <div style={{ padding: 60, textAlign: "center", color: "#64748B", fontSize: 14 }}>Loading fraud findings…</div>
          ) : shown.length === 0 ? (
                ...
                <tbody>
                  {shown.map((f, i) => (
                    <tr key={f.id}
                      ...
                      style={{ borderBottom: "1px solid #E2E8F0", background: sel?.id === f.id ? "#FF6B0008" : i % 2 === 0 ? "transparent" : "#FFFFFF", cursor: "pointer" }}>
                      <td style={{ padding: "10px 12px" }}>
                        <div style={{ fontWeight: 700, color: "#1E293B" }}>{f.player}</div>
                        <div style={{ fontSize: 10, color: "#475569", marginTop: 1 }}>{f.phone || "—"}</div>
                      </td>
                      <td style={{ padding: "10px 12px", color: "#64748B", whiteSpace: "nowrap" }}>{f.regNumber ?? "—"}</td>
                      <td style={{ padding: "10px 12px", color: "#64748B", whiteSpace: "nowrap" }}>{f.trialCity || "—"}</td>
                      <td style={{ padding: "10px 12px", color: "#64748B", whiteSpace: "nowrap" }}>{TYPE_LABEL[f.type] ?? f.type}</td>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ ...card, padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: "#1E293B" }}>{sel.player}</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #E2E8F0" }}>
                  ...
                  <span style={{ fontSize: 11, color: "#64748B" }}>{TYPE_LABEL[sel.type] ?? sel.type}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #E2E8F0" }}>
                  <span style={{ fontSize: 11, color: "#475569", fontWeight: 600 }}>Status</span>
                  <Badge status={sel.status} />
                </div>
                <div style={{ padding: "7px 0", borderBottom: "1px solid #E2E8F0" }}>
                  <div style={{ fontSize: 11, color: "#475569", fontWeight: 600, marginBottom: 4 }}>Evidence</div>
                  <div style={{ fontSize: 11, color: "#64748B", wordBreak: "break-all" }}>{evidenceOf(sel).label}</div>
              ...
              <div style={{ marginTop: 14 }}>
                ...
                <textarea value={note} onChange={e => setNote(e.target.value)} rows={3}
                  ...
                  style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#FFFFFF", color: "#1E293B", fontSize: 11, outline: "none", resize: "vertical", boxSizing: "border-box" }} />
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 10 }}>
                <button disabled={acting || sel.status === "cleared"} onClick={() => act("clear")}
                  style={{ padding: "9px 6px", borderRadius: 8, border: "1px solid #10B98140", background: "#10B98115", color: "#047857", fontWeight: 700, fontSize: 11, cursor: "pointer", opacity: acting || sel.status === "cleared" ? 0.4 : 1 }}>
                  Clear Flag
                </button>
                <button disabled={acting || sel.status === "blocked"} onClick={() => act("block")}
                  style={{ padding: "9px 6px", borderRadius: 8, border: "1px solid #8B5CF640", background: "#8B5CF615", color: "#7C3AED", fontWeight: 700, fontSize: 11, cursor: "pointer", opacity: acting || sel.status === "blocked" ? 0.4 : 1 }}>
                  Block
                </button>
                <button disabled={acting || sel.status === "flagged"} onClick={() => act("reflag")}
                  style={{ gridColumn: "1 / -1", padding: "9px 6px", borderRadius: 8, border: "1px solid #EF444440", background: "#EF444415", color: "#DC2626", fontWeight: 700, fontSize: 11, cursor: "pointer", opacity: acting || sel.status === "flagged" ? 0.4 : 1 }}>
                  Re-open (Flag Again)
                </button>
              </div>
