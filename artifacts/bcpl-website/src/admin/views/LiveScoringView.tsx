...
const TAB  = (a:boolean): React.CSSProperties => ({ padding:"7px 16px", borderRadius:8, border:"none", cursor:"pointer", fontWeight:700, fontSize:12, background:a?"linear-gradient(135deg,#FF6B00,#FF8C40)":"transparent", color:a?"#fff":"#A6B3D0" });
...
const SELECT: React.CSSProperties = { width:"100%", padding:"9px 12px", background:"#243050", border:"1px solid #33436B", borderRadius:8, color:"#F1F5F9", fontSize:13, outline:"none" };
...
export default function LiveScoringView() {
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          ...
          <div style={{ fontSize:12, color:"#A6B3D0", marginTop:2 }}>Select a match to start or manage scoring</div>
        </div>
          ...
          <div style={{ ...CARD, width:420, maxWidth:"100%", padding:28 }}>
            ...
            {teams.length<2 ? (
              <div style={{ padding:"14px 16px", background:"#F59E0B12", border:"1px solid #F59E0B40", borderRadius:10, marginBottom:20 }}>
                ...
                <div style={{ fontSize:12, color:"#C3CEE3", lineHeight:1.5 }}>Match schedule karne ke liye kam se kam 2 teams chahiye. Admin panel ke <b>Teams</b> tab me teams add karo.</div>
              </div>
            ):(
              <>
                {([["Team 1","team1"],["Team 2","team2"]] as const).map(([lbl,key])=>(
                  <div key={key} style={{ marginBottom:14 }}>
                    <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:5, fontWeight:600 }}>{lbl}</div>
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:5, fontWeight:600 }}>Venue</div>
                ...
                <div style={{ marginBottom:20 }}>
                  <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:5, fontWeight:600 }}>Date & Time</div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={()=>setShowAdd(false)} style={{ flex:1, padding:11, borderRadius:8, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", cursor:"pointer" }}>Cancel</button>
              {teams.length>=2&&(
      ...
      {matches.length===0&&(
        <div style={{ ...CARD, textAlign:"center", padding:48 }}>
          ...
          <div style={{ fontSize:13, color:"#A6B3D0" }}>"+ Add Match" se naya match schedule karo. Scoring shuru karne se pehle <b>Teams</b> tab me har team ke kam se kam 11 players hone chahiye.</div>
        </div>
      )}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {[
          ...
          <div key={group.label}>
            ...
            {group.items.map(m=>(
              <div key={m.id} style={{ ...CARD, marginBottom:8, cursor:m.status!=="completed"?"pointer":"default", borderColor:m.status==="live"?"rgba(239,68,68,.3)":"#33436B", transition:"transform .15s,border-color .15s", position:"relative", overflow:"hidden" }}
                onClick={()=>openMatch(m)}
                ...
                onMouseLeave={e=>{ (e.currentTarget as HTMLElement).style.transform=""; (e.currentTarget as HTMLElement).style.borderColor=m.status==="live"?"rgba(239,68,68,.3)":"#33436B"; }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div style={{ padding:"6px 14px", background:"#243050", borderRadius:8, fontWeight:900, fontSize:13, color:"rgba(255,255,255,0.9)" }}>VS</div>
  ...
  if(live.phase==="toss") return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
        <button onClick={()=>setLive(null)} style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", fontSize:12, cursor:"pointer" }}>← Back</button>
      ...
      <div style={{ ...CARD, maxWidth:520 }}>
        ...
        <div style={{ fontSize:13, color:"#A6B3D0", textAlign:"center", marginBottom:24 }}>{live.def.team1} <span style={{ color:"#94A3C4" }}>vs</span> {live.def.team2} · {live.def.venue}</div>
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:8, fontWeight:600 }}>TOSS WON BY</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[live.def.team1, live.def.team2].map(t=>(
              <button key={t} onClick={()=>setLive(l=>l?{...l,tossWinner:t}:l)}
                style={{ padding:"14px 12px", borderRadius:10, border:`2px solid ${live.tossWinner===t?teamColor(t):"#33436B"}`, background:live.tossWinner===t?`${teamColor(t)}18`:"#243050", color:live.tossWinner===t?teamColor(t):"#A6B3D0", fontWeight:700, fontSize:13, cursor:"pointer", transition:"all .2s" }}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div style={{ marginBottom:24 }}>
          <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:8, fontWeight:600 }}>ELECTED TO</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {(["bat","field"] as const).map(d=>(
              <button key={d} onClick={()=>setLive(l=>l?{...l,tossDec:d}:l)}
                style={{ padding:"14px 12px", borderRadius:10, border:`2px solid ${live.tossDec===d?"#FF6B00":"#33436B"}`, background:live.tossDec===d?"#FF6B0018":"#243050", color:live.tossDec===d?"#FF6B00":"#A6B3D0", fontWeight:700, fontSize:14, cursor:"pointer" }}>
      ...
      return (
        <div style={CARD}>
          ...
          {squadsLoading&&squad.length===0&&(
            <div style={{ fontSize:12, color:"#A6B3D0", padding:"12px 0" }}>Players load ho rahe hain…</div>
          )}
          {!squadsLoading&&squad.length<11&&(
            <div style={{ padding:"12px 14px", background:"#F59E0B12", border:"1px solid #F59E0B40", borderRadius:10, marginBottom:10 }}>
              ...
              <div style={{ fontSize:11, color:"#C3CEE3", lineHeight:1.5 }}>
          ...
          {squad.map((p,idx)=>{
            ...
            return (
              <div key={idx} onClick={()=>{ setSel(s=>picked?s.filter(n=>n!==p.name):s.length<11?[...s,p.name]:s); }}
                ...
                <div style={{ width:22, height:22, borderRadius:"50%", background:picked?"#FF6B00":"#33436B", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, flexShrink:0 }}>
                  {picked&&<span style={{ color:"#fff", fontWeight:900 }}>✓</span>}
                </div>
                <span style={{ flex:1, fontSize:12, color:picked?"#E2E8F0":"#C3CEE3", fontWeight:picked?700:400 }}>{p.name}</span>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          <button onClick={()=>setLive(l=>l?{...l,phase:"toss"}:l)} style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", fontSize:12, cursor:"pointer" }}>← Back</button>
          <div style={{ fontSize:16, fontWeight:800, color:"#F1F5F9" }}>Select Playing XI</div>
          <span style={{ fontSize:11, color:"#A6B3D0" }}>· {live.tossWinner} won toss · {bat} bat first</span>
          <button onClick={()=>loadSquads([live.def.team1, live.def.team2])} disabled={squadsLoading}
            style={{ marginLeft:"auto", padding:"6px 14px", borderRadius:8, border:"1px solid #33436B", background:"transparent", color:"#FF6B00", fontSize:11, fontWeight:700, cursor:squadsLoading?"wait":"pointer" }}>
        ...
        <button onClick={confirmXI} disabled={xi1sel.length!==11||xi2sel.length!==11||saving}
          style={{ padding:14, borderRadius:10, border:"none", background:xi1sel.length===11&&xi2sel.length===11?"linear-gradient(135deg,#FF6B00,#FF8C40)":"#33436B", color:xi1sel.length===11&&xi2sel.length===11?"#fff":"#94A3C4", fontSize:14, fontWeight:800, cursor:xi1sel.length===11&&xi2sel.length===11&&!saving?"pointer":"not-allowed", opacity:saving?0.7:1 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          {live.currentInnings===1
            ? <button onClick={()=>setLive(l=>l?{...l,phase:"xi"}:l)} style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", fontSize:12, cursor:"pointer" }}>← Back</button>
            : <button onClick={()=>setLive(null)} style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", fontSize:12, cursor:"pointer" }}>← All Matches</button>}
        ...
        <div style={{ ...CARD, maxWidth:560 }}>
          ...
          <div style={{ fontSize:12, color:"#A6B3D0", marginBottom:18 }}>Kaun khelega pehle? Striker, non-striker aur opening bowler chuno.</div>

          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:5, fontWeight:600 }}>STRIKER (on strike)</div>
          ...
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:5, fontWeight:600 }}>NON-STRIKER</div>
          ...
          <div style={{ marginBottom:22 }}>
            <div style={{ fontSize:11, color:"#A6B3D0", marginBottom:5, fontWeight:600 }}>OPENING BOWLER ({oinn.bowlingTeam})</div>
          ...
          <button onClick={startInnings} disabled={!valid}
            style={{ width:"100%", padding:13, borderRadius:10, border:"none", background:valid?"linear-gradient(135deg,#FF6B00,#FF8C40)":"#33436B", color:valid?"#fff":"#94A3C4", fontSize:14, fontWeight:800, cursor:valid?"pointer":"not-allowed" }}>
    ...
    return (
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.8)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
        <div style={{ ...CARD, width:"min(540px, 100%)", maxHeight:"90vh", overflowY:"auto" }}>
          ...
          {/* Dismissal type */}
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:11, color:"#A6B3D0", fontWeight:700, marginBottom:8 }}>DISMISSAL TYPE</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {TYPES.map(([t,l])=>(
                <button key={t} onClick={()=>setDm(d=>d?{...d,type:t as any}:d)}
                  style={{ padding:"7px 14px", borderRadius:8, border:`1px solid ${dm.type===t?"#EF4444":"#33436B"}`, background:dm.type===t?"#EF444420":"transparent", color:dm.type===t?"#EF4444":"#A6B3D0", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                  {l}
                </button>
              ))}
            </div>
          ...
          {["b","c","lbw","st","hw","cb"].includes(dm.type)&&(
            <div style={{ marginBottom:14, padding:"10px 14px", background:"#33436B20", borderRadius:8 }}>
              <span style={{ fontSize:11, color:"#A6B3D0" }}>Bowler: </span>
          ...
          {dm.type==="c"&&(
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, color:"#A6B3D0", fontWeight:700, marginBottom:6 }}>CAUGHT BY</div>
          ...
          {dm.type==="ro"&&(
            <>
              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:11, color:"#A6B3D0", fontWeight:700, marginBottom:6 }}>THROWN BY (Fielder)</div>
                <select value={dm.fielder} onChange={e=>setDm(d=>d?{...d,fielder:e.target.value}:d)} style={SELECT}>
                  <option value="">Select fielder…</option>
                  {fieldingXI.map(n=><option key={n} value={n}>{n}</option>)}
                </select>
              ...
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:11, color:"#A6B3D0", fontWeight:700, marginBottom:6 }}>WHO WAS RUN OUT?</div>
                <div style={{ display:"flex", gap:8 }}>
                  {([false,true] as const).map(ns=>(
                    <button key={String(ns)} onClick={()=>setDm(d=>d?{...d,nonStrikerOut:ns}:d)}
                      style={{ flex:1, padding:"8px 12px", borderRadius:8, border:`1px solid ${dm.nonStrikerOut===ns?"#F59E0B":"#33436B"}`, background:dm.nonStrikerOut===ns?"#F59E0B20":"transparent", color:dm.nonStrikerOut===ns?"#F59E0B":"#A6B3D0", fontWeight:700, fontSize:12, cursor:"pointer" }}>
          ...
          {dm.type==="st"&&(
            <div style={{ marginBottom:14, padding:"10px 14px", background:"#33436B20", borderRadius:8 }}>
              <span style={{ fontSize:11, color:"#A6B3D0" }}>Stumped by: </span>
          ...
          {dm.type!=="rh"&&remaining.length>0&&(
            <div style={{ marginBottom:20 }}>
              <div style={{ fontSize:11, color:"#A6B3D0", fontWeight:700, marginBottom:6 }}>NEXT BATSMAN IN</div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={()=>setDm(null)} style={{ flex:1, padding:11, borderRadius:9, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", cursor:"pointer" }}>Cancel</button>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ ...CARD, textAlign:"center", padding:48 }}>
          ...
          <div style={{ fontSize:13, color:"#A6B3D0", marginBottom:20 }}>Complete the toss and XI selection first to begin live scoring.</div>
  ...
  const renderScorecard = () => {
      ...
      <div>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:14 }}>
          <div>
            ...
            <div style={{ fontSize:22, fontWeight:900, color:"#FF6B00" }}>{d.totalRuns}/{d.totalWickets} <span style={{ fontSize:12, color:"#A6B3D0" }}>({fmtO(d.overs,d.balls)} ov)</span></div>
          </div>
          <div style={{ fontSize:12, color:"#A6B3D0" }}>Extras: {d.extras}{d.target?<div style={{ color:"#F59E0B" }}>Target: {d.target}</div>:null}</div>
        </div>
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
          <thead><tr style={{ borderBottom:"1px solid #33436B" }}>
            {["Batsman","Dismissal","R","B","4s","6s","SR"].map(h=><th key={h} style={{ padding:"6px 8px", textAlign:h==="Batsman"||h==="Dismissal"?"left":"right", color:"#94A3C4", fontWeight:700, fontSize:10 }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {d.batScores.filter(b=>b.balls>0||b.dismissal).map((b,i)=>(
              <tr key={i} style={{ borderBottom:"1px solid #3A4A72" }}>
                <td style={{ padding:"8px", color:b.dismissal?"#A6B3D0":"#E2E8F0", fontWeight:b.dismissal?400:700 }}>{b.name}{b.batting&&!b.dismissal?<span style={{ color:"#FF6B00" }}>*</span>:null}</td>
                <td style={{ padding:"8px", color:"#94A3C4", fontSize:10 }}>{b.dismissal||"not out"}</td>
                <td style={{ padding:"8px", textAlign:"right", color:b.runs>=50?"#F59E0B":"#E2E8F0", fontWeight:800 }}>{b.runs}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#A6B3D0" }}>{b.balls}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#3B82F6" }}>{b.fours}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#10B981" }}>{b.sixes}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#A6B3D0" }}>{b.balls?((b.runs/b.balls)*100).toFixed(1):"—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {d.fowList.length>0&&<div style={{ marginTop:12 }}>
          <div style={{ fontSize:10, color:"#94A3C4", fontWeight:700, marginBottom:6 }}>FALL OF WICKETS</div>
    ...
    const renderBowl = (d:InningsState) => (
      <div>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
          <thead><tr style={{ borderBottom:"1px solid #33436B" }}>
            {["Bowler","O","R","W","Econ","Wd","NB"].map(h=><th key={h} style={{ padding:"6px 8px", textAlign:h==="Bowler"?"left":"right", color:"#94A3C4", fontWeight:700, fontSize:10 }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {d.bowlScores.filter(b=>b.overs>0||b.balls>0).map((b,i)=>(
              <tr key={i} style={{ borderBottom:"1px solid #3A4A72" }}>
                <td style={{ padding:"8px", color:"#E2E8F0", fontWeight:600 }}>{b.name}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#C3CEE3" }}>{fmtO(b.overs,b.balls)}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#E2E8F0" }}>{b.runs}</td>
                <td style={{ padding:"8px", textAlign:"right", color:b.wickets>0?"#EF4444":"#A6B3D0", fontWeight:b.wickets>0?800:400 }}>{b.wickets}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#F59E0B" }}>{b.overs||b.balls?((b.runs/(b.overs+b.balls/6))||0).toFixed(2):"—"}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#A6B3D0" }}>{b.wides}</td>
                <td style={{ padding:"8px", textAlign:"right", color:"#A6B3D0" }}>{b.noBalls}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <div style={CARD}>
          ...
          {scTab==="bat2"  && (live.inn2?renderBat(live.inn2):<div style={{ color:"#94A3C4" }}>2nd innings not started yet</div>)}
          {scTab==="bowl2" && (live.inn2?renderBowl(live.inn2):<div style={{ color:"#94A3C4" }}>2nd innings not started yet</div>)}
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
        <button onClick={()=>{ setLive(null); refreshMatches(); }} style={{ padding:"6px 14px", borderRadius:8, border:"1px solid #33436B", background:"transparent", color:"#A6B3D0", fontSize:11, cursor:"pointer" }}>← All Matches</button>
        {[["live","🔴 Live"],["scorecard","📋 Scorecard"]] .map(([t,l])=><button key={t} onClick={()=>setMainTab(t as any)} style={TAB(mainTab===t)}>{l}</button>)}
        <div style={{ marginLeft:"auto", display:"flex", gap:8, alignItems:"center" }}>
          <span style={{ fontSize:11, color:"#94A3C4" }}>Match {live.def.matchNo} · Innings {live.currentInnings}</span>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {isDone&&(
            <div style={{ ...CARD, borderColor:"#10B98150", textAlign:"center", padding:32 }}>
              ...
              <div style={{ fontSize:12, color:"#A6B3D0", marginBottom:18 }}>
              ...
              <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
                <button onClick={()=>setMainTab("scorecard")} style={{ padding:"10px 22px", borderRadius:9, border:"1px solid #33436B", background:"transparent", color:"#C3CEE3", fontWeight:700, fontSize:12, cursor:"pointer" }}>📋 Full Scorecard</button>
          ...
          {/* Match header */}
          <div style={{ ...CARD, borderColor:isDone?"#33436B":"rgba(239,68,68,.3)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap", marginBottom:14 }}>
              <span style={{ fontSize:11, color:"#A6B3D0" }}>📍 {live.def.venue} · {live.def.date}</span>
              <span style={{ marginLeft:"auto", fontSize:11, color:"#94A3C4" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:14 }}>
              {([
                ...
                <div key={i} style={{ background:"#24305080", borderRadius:10, padding:"10px 14px", border:"1px solid #33436B" }}>
                  <div style={{ fontSize:10, color:"#94A3C4", fontWeight:700, marginBottom:4 }}>{label} · {d.battingTeam}</div>
                  <div style={{ fontSize:24, fontWeight:900, color:"#FF6B00" }}>{d.totalRuns}/{d.totalWickets}<span style={{ fontSize:11, color:"#A6B3D0" }}>{suffix}</span></div>
                  <div style={{ fontSize:11, color:"#C3CEE3" }}>{fmtO(d.overs,d.balls)} overs</div>
                </div>
              ))}
            </div>
            {/* CRR / RRR / Need */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6 }}>
              {[
                ...
                <div key={s.l} style={{ textAlign:"center", background:"#243050", borderRadius:8, padding:"8px 4px", border:"1px solid #33436B" }}>
                  <div style={{ fontSize:15, fontWeight:900, color:s.c }}>{s.v}</div>
                  <div style={{ fontSize:9, color:"#94A3C4", marginTop:2 }}>{s.l}</div>
                </div>
              ))}
            </div>
            {/* Current over balls */}
            <div style={{ marginTop:10, display:"flex", gap:5, alignItems:"center", flexWrap:"wrap" }}>
              <span style={{ fontSize:10, color:"#94A3C4" }}>Over {curInn.overs+1}:</span>
              {curInn.currentOverDeliveries.map((b,i)=>(
                <div key={i} style={{ width:26, height:26, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:800,
                  background:b==="6"?"#10B98130":b==="4"?"#3B82F630":b==="W"?"#EF444430":"#33436B",
                  color:b==="6"?"#10B981":b==="4"?"#3B82F6":b==="W"?"#EF4444":"#C3CEE3", border:"1px solid rgba(255,255,255,0.18)" }}>{b}</div>
              ))}
              {Array.from({length:Math.max(0,6-curInn.currentOverDeliveries.length)}).map((_,i)=>(
                <div key={`e${i}`} style={{ width:26, height:26, borderRadius:"50%", border:"1px dashed #33436B" }}/>
              ))}
            </div>
          </div>
          ...
          {!isDone&&<div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <div style={CARD}>
              <div style={{ fontSize:10, color:"#A6B3D0", fontWeight:700, textTransform:"uppercase", letterSpacing:.5, marginBottom:12 }}>At The Crease</div>
              {striker&&(
                <div style={{ display:"flex", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #3A4A72", marginBottom:8 }}>
                  ...
                  <span style={{ fontSize:11, color:"#94A3C4", marginLeft:4 }}>({striker.balls})</span>
              ...
              {nonStr&&(
                <div style={{ display:"flex", alignItems:"center", padding:"4px 0" }}>
                  <span style={{ width:8, height:8, borderRadius:"50%", background:"#33436B", border:"1px solid #94A3C4", marginRight:8, flexShrink:0 }}/>
                  <span style={{ flex:1, fontSize:12, color:"#C3CEE3" }}>{nonStr.name}</span>
                  <span style={{ fontSize:15, fontWeight:700, color:"#C3CEE3" }}>{nonStr.runs}</span>
                  <span style={{ fontSize:10, color:"#94A3C4", marginLeft:4 }}>({nonStr.balls})</span>
                </div>
              )}
              ...
              <div style={{ marginTop:12, display:"flex", flexDirection:"column", gap:6 }}>
                {([["Striker","strikerIdx"],["Non-Striker","nonStrikerIdx"]] as const).map(([lbl,key])=>(
                  <div key={key}>
                    <div style={{ fontSize:9, color:"#94A3C4", marginBottom:3, fontWeight:700 }}>{lbl.toUpperCase()}</div>
                    <select value={curInn[key]} onChange={e=>setBatIdx(key, Number(e.target.value))}
                      style={{ width:"100%", padding:"6px 8px", background:"#243050", border:"1px solid #33436B", borderRadius:7, color:"#F1F5F9", fontSize:11, outline:"none" }}>
            ...
            <div style={CARD}>
              <div style={{ fontSize:10, color:"#A6B3D0", fontWeight:700, textTransform:"uppercase", letterSpacing:.5, marginBottom:12 }}>Current Bowler</div>
              {bowler&&(
                <>
                  <div style={{ fontSize:14, fontWeight:700, color:"#E2E8F0", marginBottom:10 }}>{bowler.name}</div>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6, marginBottom:12 }}>
                    {[{v:fmtO(bowler.overs,bowler.balls),l:"Overs",c:"#F59E0B"},{v:bowler.runs,l:"Runs",c:"#E2E8F0"},{v:bowler.wickets,l:"Wkts",c:"#EF4444"},{v:bowler.overs||bowler.balls?((bowler.runs/(bowler.overs+bowler.balls/6))||0).toFixed(1):"—",l:"Econ",c:"#A6B3D0"}].map(s=>(
                      <div key={s.l} style={{ textAlign:"center", background:"#243050", borderRadius:7, padding:"7px 3px" }}>
                        <div style={{ fontSize:14, fontWeight:900, color:s.c }}>{s.v}</div>
                        <div style={{ fontSize:8, color:"#94A3C4", marginTop:2 }}>{s.l}</div>
                      </div>
                    ))}
                  </div>
                </>
              )}
              <div style={{ fontSize:9, color:"#94A3C4", marginBottom:4, fontWeight:700 }}>CHANGE BOWLER</div>
              <select value={curInn.bowlerIdx} onChange={e=>setLive(l=>{
                ...
                return l.currentInnings===1?{...l,inn1:upd}:{...l,inn2:upd};
              })} style={{ width:"100%", padding:"7px 10px", background:"#243050", border:"1px solid #33436B", borderRadius:8, color:"#F1F5F9", fontSize:12, outline:"none" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:isDone?"1fr":"1fr 1fr", gap:14 }}>
            {!isDone&&<div style={CARD}>
              <div style={{ fontSize:10, color:"#A6B3D0", fontWeight:700, textTransform:"uppercase", letterSpacing:.5, marginBottom:14 }}>Scoring Pad</div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:7 }}>
                {([["0","#33436B","#C3CEE3"],["1","#6E7DA640","#3B82F6"],["2","#6E7DA640","#3B82F6"],["3","#6E7DA640","#3B82F6"],["4","#3B82F620","#3B82F6"],["6","#10B98120","#10B981"],["W","#EF444420","#EF4444"],["•","#33436B","#A6B3D0"],["WD","#F59E0B20","#F59E0B"],["NB","#F59E0B20","#F59E0B"],["LB","#8B5CF620","#8B5CF6"],["B","#8B5CF620","#8B5CF6"]] as [string,string,string][]).map(([lbl,bg,tc])=>(
            ...
            <div style={CARD}>
              <div style={{ fontSize:10, color:"#A6B3D0", fontWeight:700, textTransform:"uppercase", letterSpacing:.5, marginBottom:10 }}>Commentary</div>
              <div style={{ display:"flex", gap:8, marginBottom:10 }}>
                <input value={customNote} onChange={e=>setCustomNote(e.target.value)}
                  ...
                  style={{ flex:1, padding:"8px 10px", background:"#243050", border:"1px solid #33436B", borderRadius:8, color:"#F1F5F9", fontSize:12, outline:"none" }}/>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:5, maxHeight:260, overflowY:"auto" }}>
                {commentary.map((c,i)=>(
                  <div key={i} style={{ padding:"6px 10px", background:i===0?"#FF6B0008":"#1F2B49", border:`1px solid ${i===0?"#FF6B0030":"#3A4A72"}`, borderRadius:7, fontSize:11, color:i===0?"#E2E8F0":"#A6B3D0" }}>{c}</div>
                ))}
              </div>
            </div>
          </div>
