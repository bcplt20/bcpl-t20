...
const labelStyle: React.CSSProperties = { fontSize: 11, fontWeight: 700, color: "#94A3C4", display: "block", marginBottom: 7 };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 8 }}>
              {DATASETS.map(d => (
                <button key={d.id} onClick={() => pickDataset(d.id)}
                  style={{ padding: 12, borderRadius: 10, border: `1px solid ${ds === d.id ? "#FF6B00" : "#33436B"}`, background: ds === d.id ? "#FF6B0018" : "transparent", cursor: "pointer", textAlign: "left" }}>
                  <div style={{ fontSize: 20, marginBottom: 6 }}>{d.icon}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: ds === d.id ? "#FF6B00" : "#C3CEE3" }}>{d.label}</div>
                  <div style={{ fontSize: 11, color: "#A6B3D0", marginTop: 3 }}>{d.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={card}>
            ...
            <div style={{ fontSize: 11, color: "#94A3C4", marginTop: 12 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={card}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {selDs.fields.map(f => (
                <span key={f} style={{ fontSize: 11, color: "#C3CEE3", background: "#33436B66", border: "1px solid #33436B", borderRadius: 6, padding: "3px 8px" }}>{f}</span>
              ))}
            </div>
            <button onClick={handleExport} disabled={loading}
              style={{ width: "100%", marginTop: 18, padding: "13px 0", borderRadius: 12, border: "none", background: loading ? "#8593B3" : "linear-gradient(135deg,#FF6B00,#FF8C40)", color: "#fff", fontSize: 14, fontWeight: 800, cursor: loading ? "wait" : "pointer" }}>
              {loading ? "Preparing CSV…" : "⬇ Download CSV"}
            </button>
            <div style={{ fontSize: 11, color: "#94A3C4", marginTop: 10, textAlign: "center" }}>
          ...
          <div style={card}>
            ...
            {history.length === 0 ? (
              <div style={{ fontSize: 12, color: "#94A3C4" }}>No downloads yet — pick a dataset and click Download.</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {history.map((h, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 8, fontSize: 12, color: "#C3CEE3", borderBottom: i < history.length - 1 ? "1px solid #33436B55" : "none", paddingBottom: 6 }}>
                    <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>📄 {h.name}</span>
                    <span style={{ color: "#94A3C4", flexShrink: 0 }}>{h.at}</span>
                  </div>
                ))}
              </div>
            )}
