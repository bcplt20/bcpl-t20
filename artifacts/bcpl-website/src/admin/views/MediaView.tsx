  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#1E293B" }}>Media Library</div>
      ...
      {uploads.length > 0 && (
        <div style={card}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#1E293B" }}>Uploads</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {uploads.map(u => (
              <div key={u.id}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: u.error ? "#DC2626" : "#94A3B8", marginBottom: 4 }}>
                  <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.name}</span>
                  <span>{u.error ? "failed" : u.done ? "done ✓" : `${u.pct}%`}</span>
                </div>
                {u.error ? (
                  <div style={{ fontSize: 11, color: "#DC2626" }}>{u.error}</div>
                ) : (
      ...
      {!openFolder ? (
        ...
        loading ? (
          ...
          <div style={{ ...card, textAlign: "center", padding: 48 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "#1E293B", marginBottom: 6 }}>No folders yet</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 14 }}>
            {folders.map(f => (
              <div key={f.id} style={{ ...card, padding: 18, cursor: "pointer", position: "relative" }} onClick={() => void openFolderView(f)}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: "#1E293B", marginBottom: 4, paddingRight: 20 }}>{f.name}</div>
                ...
                <button onClick={e => { e.stopPropagation(); void togglePublic(f); }}
                  ...
                  style={{ padding: "5px 12px", borderRadius: 100, border: `1px solid ${f.isPublic ? "#22C55E" : "#E2E8F0"}`, background: f.isPublic ? "#22C55E22" : "transparent", color: f.isPublic ? "#15803D" : "#64748B", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div
            ...
            style={{ ...card, border: dragOver ? "2px dashed #FF6B00" : "1px solid #E2E8F0", minHeight: 220 }}>
            {filesLoading ? (
              <div style={{ textAlign: "center", color: "#64748B", fontSize: 13, padding: 40 }}>Loading files…</div>
            ) : files.length === 0 ? (
              <div style={{ textAlign: "center", padding: 40 }}>
                <div style={{ fontSize: 36, marginBottom: 10 }}>☁</div>
                <div style={{ fontSize: 13, color: "#64748B", fontWeight: 600 }}>Drag & drop files here, or click Upload Files</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: 12 }}>
                {files.map(f => (
                  <div key={f.id} style={{ borderRadius: 12, overflow: "hidden", border: "1px solid #E2E8F0", background: "#F5F6F8", cursor: "pointer", position: "relative" }}
                    onClick={() => setPreview(f)}>
                    {f.kind === "photo" ? (
                      <img src={f.viewUrl} alt={f.name} loading="lazy" onError={() => handleMediaError(f)} style={{ width: "100%", height: 110, objectFit: "cover", display: "block", background: "#FFFFFF" }} />
                    ) : (
                      <div style={{ width: "100%", height: 110, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 34, background: "#FFFFFF" }}>🎬</div>
                    )}
                    <div style={{ padding: "8px 10px" }}>
                      <div style={{ fontSize: 11, color: "#64748B", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.name}</div>
      ...
      {newFolderOpen && (
        <div style={{ position: "fixed", inset: 0, background: "#00000088", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 20, padding: 28, width: 440, maxWidth: "92vw" }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#1E293B", marginBottom: 16 }}>New Folder</div>
            ...
            <input value={newFolderName} onChange={e => setNewFolderName(e.target.value)} onKeyDown={e => e.key === "Enter" && void createFolder()}
              ...
              style={{ width: "100%", padding: "10px 14px", background: "#F5F6F8", border: "1px solid #E2E8F0", borderRadius: 10, color: "#1E293B", fontSize: 13, outline: "none", marginBottom: 16 }} />
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
              {(["photo", "video", "mixed"] as const).map(t => (
                <button key={t} onClick={() => setNewFolderKind(t)}
                  style={{ flex: 1, padding: "9px 0", borderRadius: 9, border: `1px solid ${newFolderKind === t ? "#FF6B00" : "#E2E8F0"}`, background: newFolderKind === t ? "#FF6B0022" : "transparent", color: newFolderKind === t ? "#C2410C" : "#64748B", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
      ...
      {preview && (
        <div onClick={() => setPreview(null)}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 20, padding: 20, maxWidth: "min(880px,94vw)", maxHeight: "92vh", display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#1E293B", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{preview.name}</div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => void removeFile(preview)} style={{ ...btnGhost, color: "#DC2626", borderColor: "#EF444455", padding: "8px 14px" }}>Delete</button>
              </div>
            </div>
          </div>
        </div>
