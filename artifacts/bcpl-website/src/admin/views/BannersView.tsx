...
export default function BannersView() {
  ...
  const card: React.CSSProperties = { background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: 16, padding: "20px 22px" };
  ...
  return (
    <div style={{ padding: 28, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
        {[
          { label: "Total Banners", value: banners.length, color: "#1D4ED8", icon: "🖼" },
          { label: "Active", value: activeCount, color: "#047857", icon: "✅" },
          { label: "Inactive", value: banners.length - activeCount, color: "#475569", icon: "💤" },
          { label: "Locations", value: "8", color: "#B45309", icon: "📍" },
        ].map((s, i) => (
      ...
      {showAdd && (
        <div style={{ ...card, marginBottom: 16, borderColor: "#FF6B0030" }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: "#C2410C", marginBottom: 16 }}>New Banner</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: "#475569", letterSpacing: 0.5 }}>BANNER TITLE</label>
              <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Phase 2 Open!" style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#FFFFFF", color: "#1E293B", fontSize: 12, outline: "none", boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: "#475569", letterSpacing: 0.5 }}>LOCATION</label>
              <select value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#FFFFFF", color: "#1E293B", fontSize: 12, outline: "none", boxSizing: "border-box" }}>
            ...
            <div>
              ...
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#FFFFFF", color: "#1E293B", fontSize: 12, outline: "none", boxSizing: "border-box" }}>
            ...
            <div>
              ...
              <input value={form.cta} onChange={e => setForm(f => ({ ...f, cta: e.target.value }))} placeholder="e.g. Register Now" style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#FFFFFF", color: "#1E293B", fontSize: 12, outline: "none", boxSizing: "border-box" }} />
            </div>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
            ...
            <button onClick={() => setShowAdd(false)} style={{ padding: "9px 22px", borderRadius: 8, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      )}
          ...
          <tbody>
            {banners.map(b => (
              <tr key={b.id} style={{ borderBottom: "1px solid #E2E8F0" }}>
                <td style={{ padding: "11px 12px", color: "#64748B", fontFamily: "monospace" }}>{b.priority}</td>
                <td style={{ padding: "11px 12px", color: "#1E293B", fontWeight: 600 }}>{b.title}</td>
                ...
                <td style={{ padding: "11px 12px", color: b.cta ? "#64748B" : "#E2E8F0" }}>{b.cta || "—"}</td>
                <td style={{ padding: "11px 12px" }}><span style={{ background: b.active ? "#10B98120" : "#E2E8F0", color: b.active ? "#047857" : "#475569", padding: "2px 8px", borderRadius: 6, fontSize: 10, fontWeight: 700 }}>{b.active ? "Active" : "Inactive"}</span></td>
                <td style={{ padding: "11px 12px" }}>
                  <button onClick={() => toggle(b.id)} style={{ width: 44, height: 24, borderRadius: 12, border: "none", background: b.active ? "#10B981" : "#E2E8F0", cursor: "pointer", position: "relative", transition: "background 0.2s" }}>
