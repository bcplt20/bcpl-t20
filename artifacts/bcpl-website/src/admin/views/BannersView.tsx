...
export default function BannersView() {
  ...
  const card: React.CSSProperties = { background: "#2C3A5E", border: "1px solid #33436B", borderRadius: 16, padding: "20px 22px" };
      ...
      {showAdd && (
        <div style={{ ...card, marginBottom: 16, borderColor: "#FF6B0030" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: "#94A3C4", letterSpacing: 0.5 }}>BANNER TITLE</label>
              <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Phase 2 Open!" style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #33436B", background: "#1F2B49", color: "#E2E8F0", fontSize: 12, outline: "none", boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: "#94A3C4", letterSpacing: 0.5 }}>LOCATION</label>
              <select value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #33436B", background: "#1F2B49", color: "#E2E8F0", fontSize: 12, outline: "none", boxSizing: "border-box" }}>
            ...
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: "#94A3C4", letterSpacing: 0.5 }}>TYPE</label>
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #33436B", background: "#1F2B49", color: "#E2E8F0", fontSize: 12, outline: "none", boxSizing: "border-box" }}>
                {Object.keys(TYPE_C).map(t => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: "#94A3C4", letterSpacing: 0.5 }}>CTA TEXT</label>
              <input value={form.cta} onChange={e => setForm(f => ({ ...f, cta: e.target.value }))} placeholder="e.g. Register Now" style={{ width: "100%", marginTop: 5, padding: "9px 10px", borderRadius: 8, border: "1px solid #33436B", background: "#1F2B49", color: "#E2E8F0", fontSize: 12, outline: "none", boxSizing: "border-box" }} />
            </div>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
            ...
            <button onClick={() => setShowAdd(false)} style={{ padding: "9px 22px", borderRadius: 8, border: "1px solid #33436B", background: "transparent", color: "#A6B3D0", fontSize: 13, cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      )}
          ...
          <thead>
            <tr style={{ borderBottom: "1px solid #33436B" }}>
              {["#", "Banner Title", "Location", "Type", "CTA", "Status", "Toggle"].map(h => (
                <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "#94A3C4", letterSpacing: 0.5 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {banners.map(b => (
              <tr key={b.id} style={{ borderBottom: "1px solid #3A4A72" }}>
                <td style={{ padding: "11px 12px", color: "#8593B3", fontFamily: "monospace" }}>{b.priority}</td>
                <td style={{ padding: "11px 12px", color: "#E2E8F0", fontWeight: 600 }}>{b.title}</td>
                <td style={{ padding: "11px 12px", color: "#A6B3D0", fontSize: 11 }}>{b.location}</td>
                ...
                <td style={{ padding: "11px 12px", color: b.cta ? "#C3CEE3" : "#8593B3" }}>{b.cta || "—"}</td>
                <td style={{ padding: "11px 12px" }}><span style={{ background: b.active ? "#10B98120" : "#33436B", color: b.active ? "#10B981" : "#94A3C4", padding: "2px 8px", borderRadius: 6, fontSize: 10, fontWeight: 700 }}>{b.active ? "Active" : "Inactive"}</span></td>
                <td style={{ padding: "11px 12px" }}>
                  <button onClick={() => toggle(b.id)} style={{ width: 44, height: 24, borderRadius: 12, border: "none", background: b.active ? "#10B981" : "#33436B", cursor: "pointer", position: "relative", transition: "background 0.2s" }}>
