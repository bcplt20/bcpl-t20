  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800, color:"#1E293B" }}>Video Review Panel</div>
          <div style={{ fontSize:12, color:"#64748B", marginTop:2 }}>Review Phase 1 trial videos submitted by players</div>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          {pending > 0 && (
            <span style={{ fontSize:12, fontWeight:700, color:"#B45309", background:"#F59E0B15", padding:"6px 12px", borderRadius:8, border:"1px solid #F59E0B30" }}>
              {pending} Pending
            </span>
          )}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
        {[
          { label:"Total Videos",   value: videos.length, color:"#4F46E5" },
          { label:"Pending Review", value: pending,        color:"#B45309" },
          { label:"Reviewed",       value: reviewed,       color:"#047857" },
        ].map(s => (
      ...
      <div style={{ display:"flex", gap:6 }}>
        {[
          ...
          <button key={f.id} onClick={() => setFilter(f.id)}
            style={{ padding:"8px 18px", borderRadius:9, border:`1px solid ${filter===f.id?"#FF6B00":"#E2E8F0"}`, background:filter===f.id?"#FF6B0022":"transparent", color:filter===f.id?"#C2410C":"#64748B", fontSize:12, fontWeight:700, cursor:"pointer" }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {/* Main content */}
      {loading ? (
        <div style={{ ...card, padding:60, textAlign:"center", color:"#64748B", fontSize:14 }}>Loading videos…</div>
      ) : videos.length === 0 ? (
        <div style={{ ...card, padding:80, textAlign:"center" }}>
          <div style={{ fontSize:32, marginBottom:12 }}>🎬</div>
          <div style={{ fontSize:15, fontWeight:700, color:"#64748B" }}>No videos yet</div>
          ...
          <div style={{ display:"contents" }}>
            {videos.map(v => (
              <div key={v.id}
                ...
                style={{ ...card, cursor:"pointer", border:`1px solid ${sel?.id===v.id?"#FF6B0060":"#E2E8F0"}`, padding:16 }}>
                {/* Thumbnail / play area */}
                <div style={{ height:110, background:"linear-gradient(135deg,#FFFFFF,#E2E8F0)", borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:12, position:"relative", overflow:"hidden" }}>
                  ...
                  {v.score && (
                    <div style={{ position:"absolute", top:7, right:8, background:"#E8B23D22", borderRadius:4, padding:"2px 7px", fontSize:9, fontWeight:800, color:"#B45309", border:"1px solid #E8B23D44" }}>
                      ★ {v.score.total}/100
                    </div>
                  )}
                </div>
                <div style={{ fontSize:13, fontWeight:700, color:"#1E293B" }}>{v.player}</div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              <div style={{ ...card }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
                  <div>
                    <div style={{ fontSize:14, fontWeight:700, color:"#1E293B" }}>{sel.player}</div>
                    <div style={{ fontSize:11, color:"#475569", marginTop:2 }}>
                      {ROLE_LABEL[sel.role] ?? sel.role} · {sel.trialCity || "—"}
                    </div>
                  </div>
                  <button onClick={() => setSel(null)} style={{ background:"none", border:"none", color:"#64748B", cursor:"pointer", fontSize:18, padding:4 }}>×</button>
                </div>
                ...
                {(playUrl ?? sel.s3Url) ? (
                  <video
                    ...
                    style={{ width:"100%", borderRadius:10, background:"#E2E8F0", marginBottom:16, maxHeight:200 }}
                  />
                ) : (
                  <div style={{ height:140, background:"linear-gradient(135deg,#FFFFFF,#E2E8F0)", borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:16 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:16 }}>
                  {[
                    ...
                    <div key={item.label} style={{ display:"flex", justifyContent:"space-between", padding:"6px 0", borderBottom:"1px solid #E2E8F0" }}>
                      <span style={{ fontSize:11, color:"#475569", fontWeight:600 }}>{item.label}</span>
                      <span style={{ fontSize:11, color:"#64748B" }}>{item.value}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                  {(playUrl ?? sel.s3Url) && (
                    <a href={(playUrl ?? sel.s3Url)!} target="_blank" rel="noreferrer"
                      style={{ flex:1, padding:10, borderRadius:10, border:"1px solid #6366F140", background:"#6366F120", color:"#4F46E5", fontWeight:800, fontSize:12, cursor:"pointer", textAlign:"center", textDecoration:"none" }}>
                      ↗ Open Video
                    </a>
                  )}
                  {sel.status === "submitted" && (
                    <button
                      ...
                      style={{ flex:1, padding:10, borderRadius:10, border:"1px solid #FF6B0060", background:"#FF6B0020", color:"#C2410C", fontWeight:800, fontSize:12, cursor:"pointer", opacity:acting?0.5:1 }}>
                      👁 Mark Reviewed
                    </button>
                  )}
                  {sel.status === "reviewed" && (
                    <div style={{ flex:1, padding:10, borderRadius:10, border:"1px solid #10B98140", background:"#10B98120", color:"#047857", fontWeight:800, fontSize:12, textAlign:"center" }}>
                      ✓ Reviewed
                    </div>
                  )}
                </div>
                ...
                <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:10, padding:12 }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <button
                      ...
                      style={{ flex:1, padding:"10px 8px", borderRadius:9, border:"1px solid #22C55E60", background:"#22C55E18", color:"#15803D", fontWeight:900, fontSize:13, cursor:"pointer", opacity:acting?0.5:1 }}>
                      ✓ SELECT
                    </button>
                    <button
                      disabled={!!acting}
                      onClick={() => setP1Status(sel, "rejected")}
                      style={{ flex:1, padding:"10px 8px", borderRadius:9, border:"1px solid #EF444460", background:"#EF444418", color:"#DC2626", fontWeight:900, fontSize:13, cursor:"pointer", opacity:acting?0.5:1 }}>
                      ✗ REJECT
                    </button>
                  </div>
  ...
  return (
    <div style={{ background:"rgba(232,178,61,0.04)", border:"1px solid rgba(232,178,61,0.25)", borderRadius:10, padding:12, marginBottom:8 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <div style={{ fontSize:10, fontWeight:700, color:"#B45309", letterSpacing:".08em", textTransform:"uppercase" }}>
          ★ BCPL Score — 100-point system
        </div>
        <div style={{ fontSize:13, fontWeight:900, color: complete ? "#B45309" : "#475569", fontFamily:"Montserrat,sans-serif" }}>
          {total}/100
        </div>
      </div>
        ...
        {CRITERIA.map(c => (
          <div key={c.key}>
            <div style={{ fontSize:9.5, color:"#64748B", fontWeight:700, marginBottom:3 }}>
              {c.label} <span style={{ color:"#64748B" }}>/{c.max}</span>
            </div>
            <input
              ...
              placeholder={"0–" + c.max}
              style={{ width:"100%", padding:"8px 10px", borderRadius:8, border:"1px solid #E2E8F0", background:"#E2E8F0", color:"#1E293B", fontSize:13, fontWeight:700, outline:"none" }}
            />
          </div>
        ))}
      </div>
      ...
      <textarea
        ...
        rows={2}
        style={{ width:"100%", padding:"9px 10px", borderRadius:8, border:"1px solid #E2E8F0", background:"#E2E8F0", color:"#1E293B", fontSize:12, outline:"none", resize:"vertical", marginBottom:8, fontFamily:"inherit" }}
      />
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        <button
          ...
          onClick={save}
          style={{ padding:"9px 18px", borderRadius:8, border:"none", background: complete ? "linear-gradient(135deg,#E8B23D,#C4901E)" : "#E2E8F0", color: complete ? "#081020" : "#475569", fontWeight:900, fontSize:12, cursor: complete ? "pointer" : "not-allowed", opacity: saving ? 0.6 : 1 }}>
          {saving ? "Saving…" : video.score ? "Update Score" : "Save Score"}
        </button>
        {msg && <span style={{ fontSize:11, fontWeight:700, color: msg.startsWith("✓") ? "#047857" : "#F59E0B" }}>{msg}</span>}
      </div>
      <div style={{ fontSize:9.5, color:"#64748B", marginTop:7, lineHeight:1.5 }}>
