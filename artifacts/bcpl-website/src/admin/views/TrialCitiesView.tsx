...
export default function TrialCitiesView() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", borderBottom: "1px solid #E2E8F0", paddingBottom: 12 }}>
        {([["cities", "\ud83c\udfd9\ufe0f Cities & Venues"], ["slots", "\ud83d\uddd3\ufe0f Slots & Allocation"], ["alloc", "\ud83d\udc65 Allocations"], ["checkin", "\ud83c\udfab Check-In"], ["assess", "\ud83d\udccb Assessments"]] as const).map(([k, label]) => (
          <button key={k} onClick={() => setTtab(k)} style={{ padding: "8px 16px", borderRadius: 9, border: "1px solid " + (ttab === k ? "#F59E0B" : "#E2E8F0"), background: ttab === k ? "rgba(245,158,11,0.12)" : "transparent", color: ttab === k ? "#B45309" : "#94A3B8", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{label}</button>
        ))}
      </div>
      {ttab === "slots" && <SlotsTab />}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#1E293B" }}>Trial City Manager</div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={load} style={{ padding: "9px 16px", borderRadius: 9, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>↺ Refresh</button>
      ...
      {announceResult && (
        <div style={{ padding: 14, borderRadius: 10, background: "#10B98115", border: "1px solid #10B98140", color: "#047857", fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span>✅ Announcement sent to <strong>{announceResult.sent}</strong> of {announceResult.total} eligible Phase 2 players</span>
          <button onClick={() => setAnnounceResult(null)} style={{ background: "none", border: "none", color: "#047857", cursor: "pointer", fontSize: 16 }}>×</button>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        {[
          { label: "Total Cities",     value: total,     color: "#4F46E5" },
          { label: "Upcoming",         value: upcoming,  color: "#B45309" },
          { label: "Active",           value: active,    color: "#047857" },
          { label: "Announced",        value: announced, color: "#C2410C" },
        ].map(s => (
      ...
      {/* Venue cards */}
      {loading ? (
        <div style={{ ...card, padding: 60, textAlign: "center", color: "#64748B", fontSize: 14 }}>Loading venues…</div>
      ) : venues.length === 0 ? (
        <div style={{ ...card, padding: 80, textAlign: "center" }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>🏟️</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#64748B" }}>No trial venues added yet</div>
          ...
          {venues.map(v => (
            <div key={v.id} style={{ ...card, position: "relative" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div>
                  <div style={{ fontSize: 17, fontWeight: 800, color: "#1E293B" }}>{v.city}</div>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 5 }}>
                  ...
                  {v.announcedAt && (
                    <span style={{ fontSize: 9, color: "#047857", fontWeight: 700 }}>📢 Announced</span>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 14 }}>
                {[
                  ...
                  <div key={d.label} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8, padding: "8px 10px" }}>
                    ...
                    <div style={{ fontSize: 12, color: "#1E293B", fontWeight: 600 }}>{d.value}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                ...
                <button
                  ...
                  style={{ flex: 1, minWidth: 100, padding: "8px 10px", borderRadius: 8, border: "none", background: v.announcedAt ? "rgba(16,185,129,0.12)" : "linear-gradient(135deg,#FF6B00,#FF8C40)", color: v.announcedAt ? "#047857" : "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", opacity: acting === v.id ? 0.5 : 1 }}>
                  {acting === v.id ? "Sending…" : v.announcedAt ? "✅ Re-Announce" : "📢 Announce"}
                </button>
                {/* Edit */}
                <button onClick={() => openEdit(v)} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 11, cursor: "pointer" }}>✏ Edit</button>
                {/* Status toggle */}
                {v.status !== "completed" && (
                  <button
                    disabled={acting === v.id}
                ...
                {/* Delete */}
                <button onClick={() => setConfirmDel(v.id)} style={{ padding: "8px 10px", borderRadius: 8, border: "1px solid #EF444440", background: "transparent", color: "#DC2626", fontSize: 11, cursor: "pointer" }}>🗑</button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div style={{ ...card, width: "100%", maxWidth: 520, padding: 28 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                <button onClick={() => setAddOpen(false)} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, cursor: "pointer" }}>Cancel</button>
      ...
      {confirmDel && (
        <div style={{ position: "fixed", inset: 0, background: "#00000085", zIndex: 110, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
          ...
          <div style={{ ...card, width: "100%", maxWidth: 380, padding: 24, textAlign: "center" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "#1E293B", marginBottom: 8 }}>Delete this venue?</div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setConfirmDel(null)} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 13, cursor: "pointer" }}>Cancel</button>
