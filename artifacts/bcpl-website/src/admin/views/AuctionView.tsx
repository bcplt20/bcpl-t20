...
export default function AuctionView() {
  ...
  if (loadErr) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <div style={{ fontSize: 12, color: "#A6B3D0", marginTop: 2 }}>Franchise bidding console — SOLD players are saved to the squad instantly</div>
      </div>
      <div style={{ ...card, textAlign: "center", padding: "50px 30px" }}>
        ...
        <div style={{ fontSize: 12, color: "#A6B3D0" }}>{loadErr} — refresh to try again.</div>
      </div>
    </div>
  );

  if (pool === null) return (
    <div style={{ ...card, textAlign: "center", color: "#94A3C4", fontSize: 13, padding: "50px 20px" }}>Loading auction data…</div>
  );
  ...
  if (pool.length === 0) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#2C3A5E,#1F2B49)", border: "1px solid #33436B", borderRadius: 20, padding: "60px 40px", textAlign: "center" }}>
        <div style={{ fontSize: 48, marginBottom: 20 }}>🔨</div>
        <div style={{ fontSize: 18, fontWeight: 800, color: "#F1F5F9", marginBottom: 10 }}>No Auction-Eligible Players Yet</div>
        <div style={{ fontSize: 13, color: "#94A3C4", lineHeight: 1.7, maxWidth: 480, margin: "0 auto" }}>
          Players appear here automatically once their <strong style={{ color: "#C3CEE3" }}>Phase 2 KYC is complete</strong>.<br />
        ...
        <div style={{ marginTop: 24, fontSize: 12, color: "#8593B3" }}>
  ...
  if (done) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <div style={{ fontSize: 12, color: "#A6B3D0", marginTop: 2 }}>All {pool.length} players have gone under the hammer</div>
      </div>
      ...
      <div style={card}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3", marginBottom: 12 }}>SOLD PLAYERS (saved to squads)</div>
        {soldPlayers.length === 0 && <div style={{ fontSize: 12, color: "#8593B3" }}>No players were sold.</div>}
        {soldPlayers.map((s, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #3A4A72" }}>
            <span style={{ fontSize: 12, color: "#F1F5F9" }}>{s.player.name} <span style={{ color: "#94A3C4" }}>→ {s.team}</span></span>
            <span style={{ fontSize: 12, color: "#10B981", fontWeight: 700 }}>{fmt(s.amount)}</span>
          </div>
        ))}
        {unsold.length > 0 && (
          <>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3", margin: "16px 0 8px" }}>UNSOLD</div>
            {unsold.map((p, i) => (
              <div key={i} style={{ fontSize: 12, color: "#A6B3D0", padding: "4px 0" }}>{p.name}{p.regNumber ? ` (${p.regNumber})` : ""}</div>
            ))}
          </>
        )}
      </div>
      <div style={card}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3", marginBottom: 12 }}>TEAM SPEND</div>
        {teams.map(t => (
          <div key={t.id} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #3A4A72" }}>
            <span style={{ fontSize: 12, color: t.color, fontWeight: 700 }}>{t.name}</span>
            <span style={{ fontSize: 12, color: "#C3CEE3" }}>{t.squad} players · {fmt(t.spent)} spent · {fmt(t.budget - t.spent)} left</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#F1F5F9" }}>Live Auction</div>
          <div style={{ fontSize: 12, color: "#A6B3D0", marginTop: 2 }}>Real bidding console — <strong style={{ color: "#10B981" }}>SOLD players are saved to the franchise squad instantly</strong></div>
        </div>
        <span style={{ fontSize: 12, color: "#A6B3D0" }}>Player {currentIdx + 1}/{pool.length}</span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ ...card, textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg,#FF6B0040,#33436B)", margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 900, color: "#FF8C40" }}>
            ...
            <div style={{ fontSize: 13, color: "#C3CEE3", marginTop: 4 }}>{player!.role}{player!.city ? ` · ${player!.city}` : ""}</div>
            {player!.regNumber && <div style={{ fontSize: 12, color: "#94A3C4", marginTop: 4, fontFamily: "monospace" }}>{player!.regNumber}</div>}
            <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 14 }}>
              <div><div style={{ fontSize: 22, fontWeight: 800, color: "#F59E0B" }}>{fmt(player!.base)}</div><div style={{ fontSize: 10, color: "#94A3C4" }}>Base Price</div></div>
            </div>
          </div>

          {/* Current Bid */}
          <div style={{ ...card, textAlign: "center" }}>
            <div style={{ fontSize: 12, color: "#A6B3D0", marginBottom: 6 }}>CURRENT BID</div>
            <div style={{ fontSize: 48, fontWeight: 900, color: "#FF6B00", letterSpacing: -1 }}>{fmt(currentBid)}</div>
            {bidHistory[0] && <div style={{ fontSize: 12, color: "#C3CEE3", marginTop: 4 }}>Last bid by {bidHistory[0].team}</div>}

            {/* Timer */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 14 }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: `conic-gradient(${timer <= 10 ? "#EF4444" : "#FF6B00"} ${timer / 30 * 360}deg,#33436B 0)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#1F2B49", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: timer <= 10 ? "#EF4444" : "#FF6B00" }}>{timer}</div>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginTop: 8 }}>
              {[50000, 100000, 200000, 500000].map(j => (
                <button key={j} onClick={() => placeBid(selectedTeam, j)} disabled={saving} style={{ padding: "8px", borderRadius: 9, border: "1px solid #33436B", background: "transparent", color: "#C3CEE3", fontSize: 11, cursor: "pointer", opacity: saving ? 0.6 : 1 }}>+{fmt(j)}</button>
              ))}
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {/* Team budgets */}
          <div style={card}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3", marginBottom: 12 }}>TEAM BUDGETS (₹30L each)</div>
            {teams.map((t, i) => {
              ...
              return (
                <div key={t.id} onClick={() => setSelectedTeam(i)} style={{ marginBottom: 12, cursor: "pointer", padding: "8px 10px", borderRadius: 9, background: selectedTeam === i ? `${t.color}15` : "transparent", border: `1px solid ${selectedTeam === i ? t.color + "40" : "transparent"}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: t.color }}>{t.name.split(" ")[0]} <span style={{ color: "#94A3C4", fontWeight: 600 }}>· {t.squad}</span></span>
                    <span style={{ fontSize: 11, color: "#A6B3D0" }}>{fmt(rem)} left</span>
                  </div>
                  <div style={{ height: 4, background: "#33436B", borderRadius: 2, overflow: "hidden" }}>
          ...
          <div style={{ ...card, flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3", marginBottom: 12 }}>BID HISTORY</div>
            {bidHistory.length === 0 && <div style={{ fontSize: 11, color: "#8593B3", textAlign: "center", padding: "20px 0" }}>No bids yet</div>}
            {bidHistory.map((b, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #3A4A72" }}>
                <span style={{ fontSize: 11, color: "#C3CEE3" }}>{b.team.split(" ")[0]}</span>
          ...
          {soldPlayers.length > 0 && (
            <div style={card}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#C3CEE3", marginBottom: 10 }}>SOLD (saved to squads)</div>
              {soldPlayers.map((s, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #3A4A72" }}>
