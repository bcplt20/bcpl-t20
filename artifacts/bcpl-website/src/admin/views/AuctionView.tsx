...
export default function AuctionView() {
    ...
    Promise.all([adminGetRegistrations(), getTeams(5)])
      .then(async ([regsRes, teamsRes]) => {
        ...
        setTeams(teamsRes.teams.map((t, i) => ({
          id: t.id, name: t.name, color: t.color || "#C2410C", slug: t.slug,
          budget: BUDGET,
  ...
  if (loadErr) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        <div style={{ fontSize: 20, fontWeight: 800, color: "#1E293B" }}>Live Auction</div>
      ...
      <div style={{ ...card, textAlign: "center", padding: "50px 30px" }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#DC2626", marginBottom: 8 }}>Could not load auction data</div>
  ...
  if (pool.length === 0) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#FFFFFF,#FFFFFF)", border: "1px solid #E2E8F0", borderRadius: 20, padding: "60px 40px", textAlign: "center" }}>
        <div style={{ fontSize: 48, marginBottom: 20 }}>🔨</div>
        <div style={{ fontSize: 18, fontWeight: 800, color: "#1E293B", marginBottom: 10 }}>No Auction-Eligible Players Yet</div>
        <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.7, maxWidth: 480, margin: "0 auto" }}>
          Players appear here automatically once their <strong style={{ color: "#64748B" }}>Phase 2 KYC is complete</strong>.<br />
  ...
  if (done) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        <div style={{ fontSize: 20, fontWeight: 800, color: "#1E293B" }}>Live Auction — Complete</div>
        <div style={{ fontSize: 12, color: "#64748B", marginTop: 2 }}>All {pool.length} players have gone under the hammer</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
        {[
          { label: "Players Auctioned", value: pool.length, color: "#4F46E5" },
          { label: "Sold", value: soldPlayers.length, color: "#047857" },
          { label: "Unsold", value: unsold.length, color: "#DC2626" },
        ].map(s => (
      ...
      <div style={card}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#64748B", marginBottom: 12 }}>SOLD PLAYERS (saved to squads)</div>
        {soldPlayers.length === 0 && <div style={{ fontSize: 12, color: "#64748B" }}>No players were sold.</div>}
        {soldPlayers.map((s, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #E2E8F0" }}>
            <span style={{ fontSize: 12, color: "#1E293B" }}>{s.player.name} <span style={{ color: "#475569" }}>→ {s.team}</span></span>
            <span style={{ fontSize: 12, color: "#047857", fontWeight: 700 }}>{fmt(s.amount)}</span>
          </div>
        ))}
        {unsold.length > 0 && (
          <>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#64748B", margin: "16px 0 8px" }}>UNSOLD</div>
            {unsold.map((p, i) => (
      ...
      <div style={card}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#64748B", marginBottom: 12 }}>TEAM SPEND</div>
        {teams.map(t => (
          <div key={t.id} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #E2E8F0" }}>
            <span style={{ fontSize: 12, color: t.color, fontWeight: 700 }}>{t.name}</span>
            <span style={{ fontSize: 12, color: "#64748B" }}>{t.squad} players · {fmt(t.spent)} spent · {fmt(t.budget - t.spent)} left</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#1E293B" }}>Live Auction</div>
          <div style={{ fontSize: 12, color: "#64748B", marginTop: 2 }}>Real bidding console — <strong style={{ color: "#047857" }}>SOLD players are saved to the franchise squad instantly</strong></div>
        </div>
        <span style={{ fontSize: 12, color: "#64748B" }}>Player {currentIdx + 1}/{pool.length}</span>
      </div>

      {banner && (
        <div style={{ padding: "11px 16px", background: "#10B98115", border: "1px solid #10B98140", borderRadius: 10, color: "#047857", fontSize: 13, fontWeight: 700 }}>{banner}</div>
      )}
      {actionErr && (
        <div style={{ padding: "11px 16px", background: "#EF444412", border: "1px solid #EF444440", borderRadius: 10, color: "#DC2626", fontSize: 13 }}>{actionErr}</div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ ...card, textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg,#FF6B0040,#FFFFFF)", margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 900, color: "#C2410C" }}>
              {player!.name.split(/\s+/).map(w => w[0]).join("").slice(0, 2).toUpperCase()}
            </div>
            <div style={{ fontSize: 24, fontWeight: 900, color: "#C2410C" }}>{player!.name}</div>
            <div style={{ fontSize: 13, color: "#64748B", marginTop: 4 }}>{player!.role}{player!.city ? ` · ${player!.city}` : ""}</div>
            {player!.regNumber && <div style={{ fontSize: 12, color: "#475569", marginTop: 4, fontFamily: "monospace" }}>{player!.regNumber}</div>}
            <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 14 }}>
              <div><div style={{ fontSize: 22, fontWeight: 800, color: "#B45309" }}>{fmt(player!.base)}</div><div style={{ fontSize: 10, color: "#475569" }}>Base Price</div></div>
            </div>
          </div>
          ...
          <div style={{ ...card, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 48, fontWeight: 900, color: "#C2410C", letterSpacing: -1 }}>{fmt(currentBid)}</div>
            {bidHistory[0] && <div style={{ fontSize: 12, color: "#64748B", marginTop: 4 }}>Last bid by {bidHistory[0].team}</div>}

            {/* Timer */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 14 }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: `conic-gradient(${timer <= 10 ? "#EF4444" : "#FF6B00"} ${timer / 30 * 360}deg,#E2E8F0 0)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#FFFFFF", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: timer <= 10 ? "#DC2626" : "#FF6B00" }}>{timer}</div>
              </div>
              <button onClick={() => setRunning(r => !r)} style={{ padding: "9px 20px", borderRadius: 9, background: running ? "#EF444420" : "#10B98120", color: running ? "#DC2626" : "#10B981", fontSize: 12, fontWeight: 700, cursor: "pointer", border: `1px solid ${running ? "#EF444440" : "#10B98140"}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginTop: 8 }}>
              {[50000, 100000, 200000, 500000].map(j => (
                <button key={j} onClick={() => placeBid(selectedTeam, j)} disabled={saving} style={{ padding: "8px", borderRadius: 9, border: "1px solid #E2E8F0", background: "transparent", color: "#64748B", fontSize: 11, cursor: "pointer", opacity: saving ? 0.6 : 1 }}>+{fmt(j)}</button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              ...
              <button onClick={markUnsold} disabled={saving} style={{ flex: 1, padding: "12px", borderRadius: 10, background: "#EF444420", color: "#DC2626", fontWeight: 800, fontSize: 13, cursor: "pointer", border: "1px solid #EF444440", opacity: saving ? 0.6 : 1 }}>✗ UNSOLD</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {/* Team budgets */}
          <div style={card}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#64748B", marginBottom: 12 }}>TEAM BUDGETS (₹30L each)</div>
            {teams.map((t, i) => {
          ...
          <div style={{ ...card, flex: 1 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#64748B", marginBottom: 12 }}>BID HISTORY</div>
            {bidHistory.length === 0 && <div style={{ fontSize: 11, color: "#64748B", textAlign: "center", padding: "20px 0" }}>No bids yet</div>}
            {bidHistory.map((b, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #E2E8F0" }}>
                <span style={{ fontSize: 11, color: "#64748B" }}>{b.team.split(" ")[0]}</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#C2410C" }}>{fmt(b.amount)}</span>
              </div>
            ))}
          </div>
          ...
          {soldPlayers.length > 0 && (
            <div style={card}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#64748B", marginBottom: 10 }}>SOLD (saved to squads)</div>
              {soldPlayers.map((s, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #E2E8F0" }}>
                  <span style={{ fontSize: 11, color: "#1E293B" }}>{s.player.name}</span>
                  <span style={{ fontSize: 11, color: "#047857", fontWeight: 700 }}>{fmt(s.amount)}</span>
                </div>
              ))}
            </div>
          )}
